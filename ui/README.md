...
| Script                       | Description                                                                                                                  |
| ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `yarn stack`                 | Launch stack from scratch building blackfury.                                                                                  |
...
We need to have a different deployment cadence for `ui` code compared to our `blackfury` or blockchain code. In order to do this we need to be able to run our frontend e2e and integration tests against background builds.
...
1. Launch a single blackfury, ethereum node, and ebrelayer instance
...
1. Test your changes
   ...
   1. Change these as you require, ensure you have no docker containers running on the ports required and test by running the `yarn stack` command in the `blackfury` (**backend**) repo. This will take a few minutes as it needs to setup and migrate all the chains.
...
- Every merge to `blackfury/develop` and `blackfury/master` publishes an image based off those branches.
