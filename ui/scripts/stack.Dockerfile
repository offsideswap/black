...
COPY --from=build-go /go/bin/blackfuryd /usr/bin/blackfuryd
COPY --from=build-go /go/bin/blackgen /usr/bin/blackgen

# Required for ebrelayer
COPY --from=build-go /black/cmd/ebrelayer /black/cmd/ebrelayer
