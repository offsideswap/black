...
const blackRecipient = web3.utils.utf8ToHex(
...
contract("Security Test", function (accounts) {
  ...
  describe("BridgeBank Security", function () {
    ...
    it("should not be able to lock when contract is paused", async function () {
      ...
      await expectRevert(
        this.bridgeBank.lock(blackRecipient, NULL_ADDRESS, 100),
        "Pausable: paused"
      );
    });
    ...
    it("should not be able to burn when contract is paused", async function () {
      ...
      await expectRevert(
        this.bridgeBank.burn(blackRecipient, this.token.address, 100),
        "Pausable: paused"
      );
    });
