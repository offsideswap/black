...
contract("BridgeBank", function (accounts) {
  ...
  describe("Bridge token minting (for burned Cosmos assets)", function () {
    ...
    it("should return true if a offsideswap address prefix is correct", async function () {
      (await this.bridgeBank.verifyBlackPrefix(this.sender)).should.be.equal(true);
    })

    it("should return false if a offsideswap address has an incorrect `black` prefix", async function () {
      const incorrectBlackAddress = web3.utils.utf8ToHex(
        "eif1nx650s8q9w28f2g3t9ztxyg48ugldptuwzpace"
      );
      (await this.bridgeBank.verifyBlackPrefix(incorrectBlackAddress)).should.be.equal(false);
    })
  ...
  describe("Bridge token deposit locking (Ethereum/ERC20 assets)", function () {
    ...
    it("should not allow users to lock ERC20 tokens if the blackaddress length is incorrect", async function () {
      const invalidBlackAddress = web3.utils.utf8ToHex(
    ...
    it("should not allow users to lock ERC20 tokens if the blackaddress prefix is incorrect", async function () {
      const invalidBlackAddress = web3.utils.utf8ToHex(
  ...
  describe("Bridge token creation", function () {
    ...
    it("should burn eFury to create fury on offsideswap", async function () {
      ...
      const symbol = 'eFury'
      const amount = 100000;
      const blackAddress = "0x" + convertToHex("black12qfvgsq76eghlagyfcfyt9md2s9nunsn40zu2h");
    ...
    it("should NOT burn eFury to create fury on offsideswap if user is blocklisted", async function () {
      // Add sender to the blocklist
    ...
    it("should mint eFury to transfer Fury from offsideswap to ethereum", async function () {
      function convertToHex(str) {
        let hex = '';
        for (let i = 0; i < str.length; i++) {
            hex += '' + str.charCodeAt(i).toString(16);
      ...
      const cosmosSender = "0x" + convertToHex("black12qfvgsq76eghlagyfcfyt9md2s9nunsn40zu2h");
