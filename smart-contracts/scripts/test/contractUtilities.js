...
function buildProvider(context, argv, logging) {
    ...
    const {getRequiredEnvironmentVariable} = context.require('./blackchainUtilities');
...
async function setAllowance(context, coinDenom, amount, argv, logging, requestParameters) {
    const blackchainUtilities = context.require('./blackchainUtilities');

    if (coinDenom != blackchainUtilities.NULL_ADDRESS) {
        ...
        if (new BN(currentAllowance).lt(new BN(amount))) {
            const approveResult = await newToken.approve(argv.bridgebank_address, blackchainUtilities.SOLIDITY_MAX_INT, requestParameters);
