...
function buildProvider(context, argv, logging) {
    ...
    const {getRequiredEnvironmentVariable} = context.require('./offsideswapUtilities');
...
async function setAllowance(context, coinDenom, amount, argv, logging, requestParameters) {
    const offsideswapUtilities = context.require('./offsideswapUtilities');

    if (coinDenom != offsideswapUtilities.NULL_ADDRESS) {
        ...
        if (new BN(currentAllowance).lt(new BN(amount))) {
            const approveResult = await newToken.approve(argv.bridgebank_address, offsideswapUtilities.SOLIDITY_MAX_INT, requestParameters);
