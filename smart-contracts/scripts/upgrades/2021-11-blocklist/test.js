...
const state = {
  addresses: {
    ...
    blackRecipient: web3.utils.utf8ToHex("black1nx650s8q9w28f2g3t9ztxyg48ugldptuwzpace"),
...
async function lock({ expectedError }) {
  ...
  try {
    await state.contracts.upgradedBridgeBank
      .connect(state.signers.user1)
      .lock(state.addresses.blackRecipient, state.contracts.bridgeToken.address, state.amount, {
        value: 0,
      });
  } catch (e) {
...
async function newProphecyClaim({ signer, nonce, expectedError }) {
  ...
  try {
    await state.contracts.cosmosBridge.connect(signer).newProphecyClaim(
      2, // LOCK TYPE
      state.addresses.blackRecipient,
      nonce,
      state.addresses.user1,
      "TEST",
      state.amount
