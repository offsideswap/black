    ...
    Pausable
{
    ...
    /*
     * @dev: Modifier to only allow valid black addresses
     */
    modifier validBlackAddress(bytes memory _blackAddress) {
        require(_blackAddress.length == 42, "Invalid len");
        require(verifyBlackPrefix(_blackAddress) == true, "Invalid black address");
        _;
    }
    ...
    /*
     * @dev: function to validate if a black address has a correct prefix
     */
    function verifyBlackPrefix(bytes memory _blackAddress)
        public
        pure
        returns (bool)
    {
        bytes3 blackInHex = 0x736966;

        for (uint256 i = 0; i < blackInHex.length; i++) {
            if (blackInHex[i] != _blackAddress[i]) {
                return false;
            }
        }
        return true;
