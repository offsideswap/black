...
The CLI option for querying the swap fee rate (`blackfuryd q clp swap-fee-rate`) and setting the swap fee
rate (`blackfuryd tx clp set-swap-fee-rate`), must be renamed to `blackfuryd q clp swap-fee-params`
and `blackfuryd tx clp set-swap-fee-params` and updated to include the min fee.
