...
Blackfuryd does not currently support asymmetric liquidity adds. This document proposes a procedure
...
```
Currently blackfuryd blocks asymmetric adds. The following procedure is proposed to enable
asymmetric adds.
...
```bash
blackfuryd query clp estimate-pool-share \
  --externalAmount=0 \
