...
import (
	...
	"github.com/Blackchain/blackfury/tools/blackgen/common"
	"github.com/Blackchain/blackfury/tools/blackgen/genesis"
	"github.com/Blackchain/blackfury/tools/blackgen/network/types"
	"github.com/Blackchain/blackfury/tools/blackgen/utils"
...
func Reset(chainID, networkDir string) error {
	...
	for _, node := range nodes {
		nodeDir := fmt.Sprintf("%s/validators/%s/%s/.blackfuryd", networkDir, chainID, node.Moniker)
...
func (n *Network) generateKey(validator *Validator) error {
	output, err := n.CLI.AddKey(validator.Moniker, validator.Mnemonic, validator.Password, fmt.Sprintf("%s/%s", validator.HomeDir, ".blackfuryd"))
	if err != nil {
		return err
	}
...
func (n *Network) addValidatorKeyToSeed(validator, seedValidator *Validator) error {
	_, err := n.CLI.AddKey(validator.Moniker, validator.Mnemonic, seedValidator.Password, fmt.Sprintf("%s/%s", seedValidator.HomeDir, ".blackfuryd"))
	if err != nil {
		return err
	}
