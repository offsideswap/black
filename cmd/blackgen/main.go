...
import (
	...
	"github.com/Offsideswap/blackfury/tools/blackgen"
	"github.com/spf13/cobra"
)

func main() {
	rootCmd := &cobra.Command{Use: "blackgen"}
...
func networkCreateCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			network := blackgen.NewBlackgen(&args[0]).NewNetwork(keyringBackend)
...
func networkResetCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			blackgen.NewBlackgen(&args[0]).NetworkReset(args[1])
		},
	}
}
...
func nodeCreateCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			node := blackgen.NewBlackgen(&args[0]).NewNode(keyringBackend)
			node.Moniker = args[1]
...
func nodeResetStateCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) == 1 {
				blackgen.NewBlackgen(&args[0]).NodeReset(nil)
			} else {
				blackgen.NewBlackgen(&args[0]).NodeReset(&args[1])
			}
		},
	}
}
...
func keyGenerateMnemonicCmd() *cobra.Command {
	return &cobra.Command{
		...
		Args:  cobra.MaximumNArgs(0),
		Run: func(cmd *cobra.Command, args []string) {
			blackgen.NewBlackgen(nil).KeyGenerateMnemonic("", "")
		},
	}
}
...
func keyRecoverFromMnemonicCmd() *cobra.Command {
	return &cobra.Command{
		...
		Args:  cobra.MinimumNArgs(1),
		Run: func(cmd *cobra.Command, args []string) {
			blackgen.NewBlackgen(nil).KeyRecoverFromMnemonic(args[0])
		},
	}
}
