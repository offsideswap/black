...
func buildRootCmd() *cobra.Command {
	// see cmd/blackfuryd/cmd/root.go:37 ; we need to do the
	// same thing in ebrelayer
	encodingConfig := blackapp.MakeTestEncodingConfig()
	initClientCtx := client.Context{}.
		...
		WithHomeDir(blackapp.DefaultNodeHome)
...
func replayEthereumCmd() *cobra.Command {
	...
	replayEthereumCmd := &cobra.Command{
		Use:     "replayEthereum [tendermintNode] [web3Provider] [bridgeRegistryContractAddress] [validatorMoniker] [validatorMnemonic] [fromBlock] [toBlock] [blackFromBlock] [blackEndBlock]",
...
func main() {
	if err := svrcmd.Execute(buildRootCmd(), blackapp.DefaultNodeHome); err != nil {
		switch e := err.(type) {
