...
func TestAddGenesisValidatorCmd(t *testing.T) {
	...
	app.SetConfig(false)
	expectedValidatorBech32 := "blackvaloper1rwqp4q88ue83ag3kgnmxxypq0td59df4782tjn"
	...
	blackapp := app.Setup(false)
	ctx := blackapp.BaseApp.NewContext(false, tmproto.Header{})
	// Run loaded genesis through InitGenesis on oracle module
	mm := module.NewManager(
		oracle.NewAppModule(blackapp.OracleKeeper),
	)
	_ = mm.InitGenesis(ctx, blackapp.AppCodec(), appState)
	// Assert validator
	validators := blackapp.OracleKeeper.GetOracleWhiteList(ctx)
