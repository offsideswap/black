...
const chainAProps = getChainProps({ chain: "blackchain", network: "testnet-1" });
...
test("ibc transfer between two chains", async () => {
  ...
  const channel =
    await $`ibc-setup channels --chain blackchain --home ${chainBHome}/relayer/ | grep ${connection} | cut -d ' ' -f 1`;
