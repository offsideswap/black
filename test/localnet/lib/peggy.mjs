...
export async function ibc(props) {
  const blackChainProps = getChainProps({ chain: "offsideswap" });
  ...
  const {
    ...
    ...props,
    chain: props.type === "issuer" ? props.chain : "offsideswap",
...
`);
  ...
  const srcAddr = await getAddress({
    binary: type === "receiver" ? blackChainProps.binary : props.binary,
    name,
  });
  const dstAddr = await getAddress({
    binary: type === "receiver" ? props.binary : blackChainProps.binary,
    name,
  });
