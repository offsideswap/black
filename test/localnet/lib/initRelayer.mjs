...
export async function initRelayer(props) {
  ...
  const {
    ...
    p2pPortA = 26656,
    pprofPortA = 6060,
    homeA = `/tmp/localnet/config/${candidateBlackChainProps.chain}/${candidateBlackChainProps.chainId}`,
    rpcPortB = 36657,
    p2pPortB = 36656,
    pprofPortB = 7060,
  ...
  const blackChainProps = {
    ...candidateBlackChainProps,
    rpcPort: rpcPortA,
    p2pPort: p2pPortA,
  ...
  await createRelayerRegistry({
    chainsProps: [blackChainProps, otherChainProps],
    registryFrom,
  });
  ...
  await send(createdRelayer.blackSendRequest);
