...
$(blackfuryd keys show user2 -a) $(blackfuryd keys show user1 -a --bech val) 10000 lock \
...
blackfuryd q auth account $(blackfuryd keys show user2 -a)
...
blackfuryd tx ethbridge burn $(blackfuryd keys show user2 -a) 0x11111111262b236c9ac9a9a8c8e4276b5cf6b2c9 \
...
## Case 3
## 1. lock user2 rwn in offsideswap
blackfuryd tx ethbridge lock $(blackfuryd keys show user2 -a) 0x11111111262b236c9ac9a9a8c8e4276b5cf6b2c9 \
...
$(blackfuryd keys show user2 -a) $(blackfuryd keys show user1 -a --bech val) \
...
sleep 8
yes Y | blackfuryd tx clp create-pool --from user2 --symbol ceth --nativeAmount 200 --externalAmount 200
sleep 8
yes Y | blackfuryd tx clp create-pool --from user2 --symbol cdash --nativeAmount 100 --externalAmount 100
...
sleep 8
blackfuryd query clp lp ceth $(blackfuryd keys show user2 -a)

echo "adding more liquidity"
sleep 8
yes Y | blackfuryd tx clp add-liquidity --from user2 --symbol ceth --nativeAmount 1 --externalAmount 1

echo "swap"
sleep 8
yes Y |  blackfuryd tx clp swap --from user2 --sentSymbol ceth --receivedSymbol cdash --sentAmount 20 ---minReceivingAmount 0


echo "removing Liquidity"
sleep 8
yes Y | blackfuryd tx clp remove-liquidity --from user2 --symbol ceth --wBasis 5001 --asymmetry -1
...
sleep 8
yes Y | blackfuryd tx clp decommission-pool --from user2 --symbol ceth

echo "blackfuryd query clp pools -> should list both pools / Decommission can only be done by admin users"