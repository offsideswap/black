...
# Blackchain public network endpoints
BETANET = {"node": "https://rpc.blackchain.finance", "chain_id": "blackchain-1"}
TESTNET = {"node": "https://rpc-testnet.blackchain.finance", "chain_id": "blackchain-testnet-1"}
DEVNET = {"node": "https://rpc-devnet.blackchain.finance", "chain_id": "blackchain-devnet-1"}
...
# Fees for blackchain -> blackchain transactions, paid by the sender.
...
def blackchain_denom_hash(network_descriptor: int, token_contract_address: eth.Address) -> str:
...
def blackchain_denom_hash_to_token_contract_address(token_hash: str) -> Tuple[int, eth.Address]:
    m = re.match("^blackBridge(\\d{4})0x([0-9a-fA-F]{40})$", token_hash)
    if not m:
        raise Exception("Invalid blackchain denom '{}'".format(token_hash))
...
class Blackfuryd:
    ...
    def create_tokenregistry_entry(self, symbol: str, blackchain_symbol: str, decimals: int,
        ...
        return {
            ...
            "denom": blackchain_symbol,
            "base_denom": blackchain_symbol,
            "path": "",
            "ibc_channel_id": "",
    ...
    # See https://github.com/Blackchain/blackfury/blob/master/docs/tutorials/clp%20tutorial.md
...
class BlackfuryClient:
    ...
    def send_from_blackchain_to_ethereum(self, from_black_addr: cosmos.Address, to_eth_addr: str, amount: int, denom: str,
        generate_only: bool = False
    ) -> Mapping:
        """ Sends ETH from Blackchain to Ethereum (burn) """
        assert self.ctx.eth
        eth = self.ctx.eth
        ...
        if on_peggy2_branch:
            ...
            # blackfuryd tx ethbridge <direction> <node> <blackchain_addr> <ethereum_addr> <amount> <symbol> <keyring backend> <ethereum-chain-id>


    def send_from_blackchain_to_ethereum_grpc(self, from_black_addr: cosmos.Address, to_eth_addr: str, amount: int,
        denom: str
    ):
        tx = self.send_from_blackchain_to_ethereum(from_black_addr, to_eth_addr, amount, denom, generate_only=True)
    ...
    def open_grpc_channel(self) -> grpc.Channel:
        ...
        # See https://raw.githubusercontent.com/Blackchain/blackchain-ui/develop/ui/core/swagger.yaml
