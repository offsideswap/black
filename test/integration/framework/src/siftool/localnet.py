...
def run_localnet_hook():
    ...
    # rm -rf /tmp/localnet/config/blackchain/blackchain-testnet-1
    # mkdir -p /tmp/localnet/config/blackchain/blackchain-testnet-1
    # /tmp/localnet/bin/blackfuryd init blackchain-testnet-1 --chain-id blackchain-testnet-1 --home /tmp/localnet/config/blackchain/blackchain-testnet-1
    # /tmp/localnet/bin/blackfuryd keys add blackchain-validator --keyring-backend test --home /tmp/localnet/config/blackchain/blackchain-testnet-1
    # /tmp/localnet/bin/blackfuryd keys add blackchain-source --keyring-backend test --home /tmp/localnet/config/blackchain/blackchain-testnet-1
    # /tmp/localnet/bin/blackfuryd add-genesis-account blackchain-validator 10000000000000000000fury --keyring-backend test --home /tmp/localnet/config/blackchain/blackchain-testnet-1
    # /tmp/localnet/bin/blackfuryd add-genesis-account blackchain-source 10000000000000000000fury --keyring-backend test --home /tmp/localnet/config/blackchain/blackchain-testnet-1
    ...
    #
    # if blackchain: ${binPath}/${binary} set-gen-denom-whitelist ${home}/config/denoms.json --home ${home}
    #
...
def download_ibc_binaries(cmd, chains_to_download=None, output_path=None):
    ...
    config = get_localnet_config(cmd)
    tmpdir = cmd.mktempdir()
    # We prefer to compile blackchain. Sentinel uses sourceUrl, but there is no Makefile.
    all_supported_chains = set(config.keys()).difference({"blackchain", "sentinel"})
