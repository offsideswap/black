...
from blacktool.blackchain import Blackgen, Blackfuryd, Ebrelayer, blackchain_denom_hash, FURY
...
class Integrator(Ganache, Command):
    ...
    # Peggy1 only
    def blackchain_init_integration(self, blackfury, validator_moniker, validator_mnemonic, denom_whitelist_file):
        ...
        assert valoper == blackfury0.get_val_address(validator_moniker)  # This does not use "home"; if it the assertion holds it could be grouped with blackchain_init_peggy

        # This was deleted in commit f00242302dd226bc9c3060fb78b3de771e3ff429 from blackchain_start_daemon.sh because
...
class UIStackEnvironment:
    def __init__(self, cmd):
        ...
        self.chain_id = "blackchain-local"
    ...
    def stack_push(self):
        ...
        image_root = "ghcr.io/blackchain/blackfury/ui-stack"
...
class IntegrationTestsEnvironment:
    ...
    def run(self):
        ...
        # # TODO This should be last (after return from setup_blackchain.sh)
        # burn_limits = [
        #     [eth.NULL_ADDRESS, 31*10**18],
        ...
        # test/integration/setup_blackchain.sh:
        ...
        adminuser_addr = self.cmd.blackchain_init_integration(blackfury, validator1_moniker, validator1_mnemonic,
        ...
        # in blackchain_start_daemon.sh: "sleep 10"
        # in blackchain_run_ebrelayer.sh (also run_ebrelayer here) we already wait for connection to port 26657 and black account validator1_addr
        ...
        # test/integration/blackchain_start_ebrelayer.sh -> test/integration/blackchain_run_ebrelayer.sh
        # This script is also called from tests

        relayer_db_path = os.path.join(self.test_integration_dir, "blackchainrelayerdb")
        ...
        self.state_vars = {
            ...
            "EBRELAYER_ETHEREUM_PRIVATE_KEY": ebrelayer_ethereum_private_key,  # Needed by blackchain_run_ebrelayer.sh
            ...
            "BLACKCHAIN_ADMIN_ACCOUNT": adminuser_addr,  # Needed by test_peggy_fees.py (via conftest.py)
            "EBRELAYER_DB": relayer_db_path,  # Created by blackchain_run_ebrelayer.sh, does not appear to be used anywhere at the moment
        }
    ...
    def create_snapshot(self, snapshot_name):
        ...
        self.cmd.tar_create(self.state_vars["EBRELAYER_DB"], os.path.join(named_snapshot_dir, "blackchainrelayerdb.tar.gz"))
    ...
    def restore_snapshot(self, snapshot_name):
        ...
        assert os.path.realpath(relayer_db_path) == os.path.realpath(os.path.join(self.test_integration_dir, "blackchainrelayerdb"))
        extract("ganache.tar.gz", ganache_db_dir)
        extract("blackchainrelayerdb.tar.gz", relayer_db_path)
...
class Peggy2Environment(IntegrationTestsEnvironment):
    def __init__(self, cmd: Command, witness_count: int = 1, witness_power: int = 100, consensus_threshold: int = 49,
        ...
        self.ceth_symbol = blackchain_denom_hash(self.ethereum_chain_id, eth.NULL_ADDRESS)
    ...
    def run(self) -> Tuple[subprocess.Popen, subprocess.Popen, subprocess.Popen, List[subprocess.Popen]]:
        ...
        # - build_golang_binaries before start_blackchain
        # - start_hardhat before deploy_smart_contracts
        # - start_blackchain before start_witnesses_and_relayers
        ...
        network_config_file, blackfuryd_exec_args, blackfuryd_proc, tcp_url, admin_account_address, blackfury_validators, \
            blackfury_relayers, blackfury_witnesses, blackfury_validator0_home, chain_dir = \
                self.init_blackchain(blackfuryd_network_dir, blackfuryd_log_file, self.validator_mint_amounts,
    ...
    def init_blackchain(self, blackfuryd_network_dir: str, blackfuryd_log_file: TextIO,
        ...
        for config in launch_json["configurations"]:
            if config["name"].startswith("Debug Relayer"):
                intellij_ebrelayer_configs.append(render_intellij_run_xml(
                    "ebrelayer devenv",
                    " ".join(config["args"]),
                    "github.com/Blackchain/blackfury/cmd/ebrelayer",
                ...
                intellij_blackfuryd_configs.append(render_intellij_run_xml(
                    "blackfuryd devenv",
                    " ".join(config["args"]),
                    "github.com/Blackchain/blackfury/cmd/blackfuryd",
