...
# The original script had a lot of problems as described in https://app.zenhub.com/workspaces/current-sprint---engineering-615a2e9fe2abd5001befc7f9/issues/blackchain/issues/719.
# See https://www.notion.so/blackchain/TEST-TOKEN-DISTRIBUTION-PROCESS-41ad0861560c4be58918838dbd292497
...
class InflateTokens:
    ...
    def build_list_of_tokens_to_create(self, existing_tokens: Iterable[TokenDict], requested_tokens: Iterable[TokenDict]
        ...
        """
        This part deploys BlackchainTestoken for every requested token that has not yet been deployed.
    ...
    def transfer_from_eth_to_blackfury(self, from_eth_addr, to_black_addr, tokens_to_transfer, amount_in_tokens, amount_eth_gwei):
        black_balances_before = self.ctx.get_blackchain_balance(to_black_addr)
        sent_amounts = []
        pending_txs = []
    ...
    def distribute_tokens_to_wallets(self, from_black_account, tokens_to_transfer, amount_in_tokens, target_black_accounts, amount_eth_gwei):
        ...
        for black_acct in target_black_accounts:
            ...
            while remaining:
                ...
                black_balance_before = self.ctx.get_blackchain_balance(black_acct)
                self.ctx.send_from_blackchain_to_blackchain(from_black_account, black_acct, batch)
    ...
    def transfer(self, requested_tokens: Sequence[TokenDict], token_amount: int,
        ...
        The black_broker_account and OPERATOR can be any Blackchain and Ethereum accounts, we might want to use something
        ...
        black_broker_account = self.ctx.create_blackchain_addr(fund_amounts=[fund_fury])
        ...
        log.info("To see newly minted tokens in UI, you need to edit 'scripts/ibc/tokenregistry/generate-erc20-jsons.sh' "
            ...
            "and commit the results in the blackchain-devnet-1 folder. @tim will pick up the PR and register it on "
