...
from blacktool import blackchain, command, cosmos
...
class BlackfurydEnvironment:
    def __init__(self, cmd: command.Command, chain_id: Optional[str] = None, blackfuryd_home_root: Optional[str] = None):
        ...
        self.open_log_files = []
        self._state = 0
        self.blackfuryd = blackchain.Blackfuryd(self.cmd, home=self.keyring_dir, chain_id=self.chain_id)
    ...
    def start(self):
        ...
        self.blackfuryd = blackchain.Blackfuryd(self.cmd, home=self.keyring_dir, chain_id=self.chain_id,
            node=blackchain.format_node_url(self.node_info[0]["host"], self.node_info[0]["ports"]["rpc"]),
    ...
    def _update_configuration_files(self, node_info, peers_node_info):
        ...
        peers = [blackchain.format_peer_address(i["node_id"], LOCALHOST, i["ports"]["p2p"])
        ...
        app_toml["api"]["address"] = blackchain.format_node_url(ANY_ADDR, node_info["ports"]["api"])
    ...
    def _blackfuryd_for(self, node_info: JsonDict, to_node_info: Optional[JsonDict] = None) -> blackchain.Blackfuryd:
        binary = node_info["binary"]
        home = node_info["home"]
        to_node_info = to_node_info if to_node_info is not None else node_info
        node = blackchain.format_node_url(to_node_info["host"], to_node_info["ports"]["rpc"])
        return blackchain.Blackfuryd(self.cmd, binary=binary, home=home, chain_id=self.chain_id, node=node)
    ...
    def _broadcast_create_validator_msg(self, node_info: JsonDict):
        ...
        assert cosmos.balance_exceeds(admin_balance, {FURY: blackchain.black_tx_fee_in_fury}), \
    ...
    def _create_validator_home(self, node_info: JsonDict):
        ...
        blackfuryd = blackchain.Blackfuryd(self.cmd, home=self.keyring_dir)
    ...
    def setup_liquidity_pools_simple(self, pool_definitions: Mapping[str, Tuple[int, int, int]]):
        ...
        if self.faucet != self.clp_admin:
            ...
            blackfuryd.send_batch(self.faucet, self.clp_admin, cosmos.balance_add(
                ...
                {blackchain.FURY: sum(native_amount for _, (_, native_amount, _) in pool_definitions.items())}))
        ...
        # "token not supported by blackchain"
        # Note: original_entry = {
