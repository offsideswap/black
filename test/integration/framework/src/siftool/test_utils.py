...
from blacktool import eth, truffle, hardhat, run_env, blackchain, cosmos, command
from blacktool.blackchain import FURY, CETH
...
def get_env_ctx(cmd=None, env_file=None, env_vars=None):
    ...
    # Add any Blackchain private keys to test keystore
...
def get_env_ctx_peggy2():
    ...
    if "BLACKTOOL_ENV_FILE" in os.environ:
        ...
        blackchain_config = env_vars["blackchain"]
        blackfury_url = blackchain_config["rpc_url"]
        blackfury_chain_id = blackchain_config["chain_id"]
        blackfuryd_home = blackchain_config.get("home")
        ...
        fury_source = blackchain_config.get("fury_source")
        if not fury_source:
            if "fury_source_mnemonic" in blackchain_config:
                fury_source_mnemonic = blackchain_config["fury_source_mnemonic"]
            ...
            if fury_source_mnemonic:
                ...
                fury_source = blackchain.mnemonic_to_address(cmd, fury_source_mnemonic)
                blackfuryd = blackchain.Blackfuryd(cmd, home=blackfuryd_home)
    ...
    ceth_symbol = blackchain.blackchain_denom_hash(ethereum_network_descriptor, eth.NULL_ADDRESS)
    ...
    # TODO These are set in run_env.py:Peggy2Environment.init_blackchain(), specifically "blackfuryd tx ethbridge set-cross-chain-fee"
...
def get_env_ctx_peggy1(cmd=None, env_file=None, env_vars=None):
    ...
    generic_erc20_contract_name = "BlackchainTestToken"
    if "SMART_CONTRACT_ARTIFACT_DIR" in env_vars:
        ...
        if deployment_name == "blackchain-1":
            # Special case for Betanet because BlackchainTestToken is not deployed there.
...
class EnvCtx:
    def __init__(self, cmd: command.Command, w3_conn: web3.Web3, ctx_eth: eth.EthereumTxWrapper, abi_provider,
        ...
        self.blackfury = blackchain.Blackfuryd(self.cmd, home=blackfuryd_home, node=blackfury_url, chain_id=blackfury_chain_id)
        # Refactoring in progress: moving stuff into separate client that encapsulates things like url, home and chain_id
        self.blackfury_client = blackchain.BlackfuryClient(self, self.blackfury, grpc_port=9090)
        ...
        self.blackchain_ethbridge_admin_account = self.fury_source
    ...
    def tx_deploy_new_generic_erc20_token(self, deployer_addr: str, name: str, symbol: str, decimals: int, cosmosDenom: str = None) -> Contract:
        # return self.tx_deploy("BlackchainTestToken", self.operator, [name, symbol, decimals])
        if on_peggy2_branch:
            # Use BridgeToken
            assert self.generic_erc20_contract == "BridgeToken"
            if cosmosDenom is None:
                cosmosDenom = "erc20denom"  # TODO Dummy variable since we're using BridgeToken instead of BlackchainTestToken

            constructor_args = [name, symbol, decimals, cosmosDenom]
        else:
            # Use BlackchainTestToken for TestNet and Devnet, and BridgeToken for Betanet
    ...
    def create_new_currency(self, symbol, name, decimals, amount, minted_tokens_recipient):
        ...
        1. Deploys a new instance of BlackchainTestToken
        ...
        """
        assert self.generic_erc20_contract == "BlackchainTestToken"  # Preserve compatibiliy with integration tests and inflate_tokens.sh
    ...
    def send_eth_from_ethereum_to_blackchain(self, from_eth_addr, to_black_addr, amount):
    ...
    # TODO Obsolete, use self.bridge_bank_lock_eth()
    def send_erc20_from_ethereum_to_blackchain(self, from_eth_addr, dest_sichain_addr, erc20_token_addr, amount):
    ...
    def create_blackchain_addr(self, moniker: str = None,
        fund_amounts: Union[cosmos.Balance, cosmos.LegacyBalance, None] = None
    ) -> cosmos.Address:
        """
        Generates a new blackchain address in test keyring. If moniker is given, uses it, otherwise
        generates a random one 'test-xxx'. If fund_amounts is given, the blackchain funds are transferred
        ...
        if fund_amounts:
            ...
            fury_source_balances = self.get_blackchain_balance(self.fury_source)
            ...
            old_balances = self.get_blackchain_balance(black_address)
            self.send_from_blackchain_to_blackchain(self.fury_source, black_address, fund_amounts)
            self.blackfury.wait_for_balance_change(black_address, old_balances, min_changes=fund_amounts)
            new_balances = self.get_blackchain_balance(black_address)
            assert cosmos.balance_zero(cosmos.balance_sub(new_balances, fund_amounts))
        return black_address

    # TODO Clean up
    def send_from_blackchain_to_blackchain(self, from_black_addr: cosmos.Address, to_black_addr: cosmos.Address,
        amounts: cosmos.Balance
    ) -> Mapping:
        return self.blackfury.send(from_black_addr, to_black_addr, amounts)

    # TODO Clean up
    def get_blackchain_balance(self, black_addr: cosmos.Address, height: Optional[int] = None,
    ...
    def eth_symbol_to_black_symbol(self, eth_token_symbol):
        assert not on_peggy2_branch
        # TODO blackchain.use blackchain_denom_hash() if on_peggy2_branch
        # E.g. "usdt" -> "cusdt"
    ...
    def bridge_bank_lock_eth(self, from_eth_acct, to_black_acct, amount):
        """ Sends ETH from Ethereum to Blackchain (lock) """
    ...
    # For Blackchain-native assets (fury) we need to use "burn".
    # Compare: smart-contracts/scripts/test/{sendLockTx.js OR sendBurnTx.js}
    # sendBurnTx is called when blackchain_symbol == "fury", sendLockTx otherwise
    def send_from_ethereum_to_blackchain(self, from_eth_acct: str, to_black_acct: str, amount: int, token_sc: Contract = None, isLock: bool = True) -> TxReceipt:
        if token_sc is None:
            # ETH transfer
    ...
    def sanity_check(self):
        ...
        if on_peggy2_branch:
            ...
            assert (self.blackfury.chain_id != "blackchain-testnet-1") or (bridge_bank_sc.address == "0x6CfD69783E3fFb44CBaaFF7F509a4fcF0d8e2835")
            assert (self.blackfury.chain_id != "blackchain-devnet-1") or (bridge_bank_sc.address == "0x96DC6f02C66Bbf2dfbA934b8DafE7B2c08715A73")
        ...
        if len(fury_source_account) != 1:
            raise Exception
        fury_source_balance = self.get_blackchain_balance(self.fury_source).get(FURY, 0)
