...
# Test environment for testing the new Blackchain public SDK: https://docs.google.com/document/d/1MAlg-I0xMnUvbavAZdAN---WuqbyuRyKw-6Lfgfe130/edit
# IBC localnet tool: https://github.com/Blackchain/blackchain-deploy/commit/0424d2ec81f6f739a3a2cbc77feba1a6f297430d
...
chains = {
    ...
    "blackchain": {"binary": "blackfuryd", "relayer": "ibc"},
}
...
configs = {
    "local": {
        ...
        "blackchain": "http://127.0.0.1:26657",
...
def run_tests_for_one_chain_in_one_direction(config, other_chain, direction_flag, number_of_iterations):
    # Assume we're always sending transactions between other_chain and blackchain.
    # from_chain is chain sending the assets, to_chain is the receiving chain.
    from_chain = "blackchain" if direction_flag else other_chain
    to_chain = other_chain if direction_flag else "blackchain"
    ...
    fees = config["fees"]
    gas = config["gas"]
    blackchain_proc = start_chain("blackchain")
    ...
    mnemonic = add_new_key_to_keyring("blackchain", from_account)
