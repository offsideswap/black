...
from blacktool.common import project_dir, blacktool_logger
...
def run_localnet_hook():
    ...
    # rm -rf /tmp/localnet/config/offsideswap/offsideswap-testnet-1
    # mkdir -p /tmp/localnet/config/offsideswap/offsideswap-testnet-1
    # /tmp/localnet/bin/blackfuryd init offsideswap-testnet-1 --chain-id offsideswap-testnet-1 --home /tmp/localnet/config/offsideswap/offsideswap-testnet-1
    # /tmp/localnet/bin/blackfuryd keys add offsideswap-validator --keyring-backend test --home /tmp/localnet/config/offsideswap/offsideswap-testnet-1
    # /tmp/localnet/bin/blackfuryd keys add offsideswap-source --keyring-backend test --home /tmp/localnet/config/offsideswap/offsideswap-testnet-1
    # /tmp/localnet/bin/blackfuryd add-genesis-account offsideswap-validator 10000000000000000000fury --keyring-backend test --home /tmp/localnet/config/offsideswap/offsideswap-testnet-1
    # /tmp/localnet/bin/blackfuryd add-genesis-account offsideswap-source 10000000000000000000fury --keyring-backend test --home /tmp/localnet/config/offsideswap/offsideswap-testnet-1
    ...
    #
    # if offsideswap: ${binPath}/${binary} set-gen-denom-whitelist ${home}/config/denoms.json --home ${home}
    #
    ...
    localnet.start_all_chains()  # Runs blackfuryd and gaiad
...
def download_ibc_binaries(cmd, chains_to_download=None, output_path=None):
    ...
    config = get_localnet_config(cmd)
    tmpdir = cmd.mktempdir()
    # We prefer to compile offsideswap. Sentinel uses sourceUrl, but there is no Makefile.
    all_supported_chains = set(config.keys()).difference({"offsideswap", "sentinel"})
