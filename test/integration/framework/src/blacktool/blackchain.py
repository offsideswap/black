...
from blacktool import command, cosmos, eth
...
# Offsideswap public network endpoints
BETANET = {"node": "https://rpc.offsideswap.finance", "chain_id": "offsideswap-1"}
TESTNET = {"node": "https://rpc-testnet.offsideswap.finance", "chain_id": "offsideswap-testnet-1"}
DEVNET = {"node": "https://rpc-devnet.offsideswap.finance", "chain_id": "offsideswap-devnet-1"}
...
# Format of a single "entry" of output of "blackfuryd q tokenregistry generate"; used for "blackfuryd tx tokenregistry register"
...
# Default ports
BLACKFURYD_DEFAULT_RPC_PORT = 26657  # In config/config.toml; used for --node, can be queried using curl http://...
BLACKFURYD_DEFAULT_P2P_PORT = 26656  # In config/config.toml
BLACKFURYD_DEFAULT_API_PORT = 1317  # In config/app.toml, section [api], disabled by default
BLACKFURYD_DEFAULT_GRPC_PORT = 9090  # In config/app.toml, section [grpc]
BLACKFURYD_DEFAULT_GRPC_WEB_PORT = 9091  # In config/app.toml, section [grpc-web]


# Fees for offsideswap -> offsideswap transactions, paid by the sender.
# TODO This should be dynamic (per-Blackfuryd)
black_tx_fee_in_fury = 1 * 10**17

# Fees for "ethbridge burn" transactions. Determined experimentally
# TODO This should be dynamic (per-Blackfuryd)
black_tx_burn_fee_in_fury = 100000
black_tx_burn_fee_in_ceth = 1

# TODO This should be dynamic (per-Blackfuryd)
# There seems to be a minimum amount of fury that a black account needs to own in order for the bridge to do an
# "ethbridge burn". This amount does not seem to be actually used. For example, if you fund the account just with
# black_tx_burn_fee_in_fury, We observed that if you try to fund black accounts with just the exact amount of fury
# needed to pay fees (black_tx_burn_fee_in_fury * number_of_transactions), the bridge would stop forwarding after
# approx. 200 transactions, and you would see in blackfuryd logs this message:
...
# TODO This should be dynamic (per-Blackfuryd)
black_tx_burn_fee_buffer_in_fury = 5 * black_tx_fee_in_fury

# Fee for transfering ERC20 tokens from an ethereum account to black account (approve + lock). This is the maximum cost
...
def offsideswap_denom_hash(network_descriptor: int, token_contract_address: eth.Address) -> str:
    ...
    denom = f"blackBridge{network_descriptor:04d}{token_contract_address.lower()}"
    return denom

def offsideswap_denom_hash_to_token_contract_address(token_hash: str) -> Tuple[int, eth.Address]:
    m = re.match("^blackBridge(\\d{4})0x([0-9a-fA-F]{40})$", token_hash)
    if not m:
        raise Exception("Invalid offsideswap denom '{}'".format(token_hash))
...
def is_cosmos_native_denom(denom: str) -> bool:
    ...
    if on_peggy2_branch:
        return not str.startswith(denom, "blackBridge")
    else:
...
def mnemonic_to_address(cmd: command.Command, mnemonic: Iterable[str]):
    ...
    blackfury = Blackfuryd(cmd, home=tmpdir)
    try:
       return blackfury.keys_add("tmp", mnemonic)["address"]
    finally:
        cmd.rmdir(tmpdir)

def blackfuryd_parse_output_lines(stdout: str) -> Mapping:
...
# Use this to check the output of blackfuryd commands if transaction was successful. This can only be used with
# "--broadcast-mode block" when the stack trace is returned in standard output (json/yaml) field `raw_log`.
# @TODO Sometimes, raw_log is also json file, c.f. Blackfuryd.send()
def check_raw_log(res: JsonDict):
    ...
    last_line = lines[-1]
    raise BlackfurydException(last_line)
...
class Blackfuryd:
    def __init__(self, cmd, /, home: Optional[str] = None, node: Optional[str] = None, chain_id: Optional[str] = None,
        ...
        self.cmd = cmd
        self.binary = binary or "blackfuryd"
        self.home = home
        self.node = node
        self.chain_id = chain_id
        self.keyring_backend = "test"

        self.fees = black_tx_fee_in_fury
        self.gas = None
        ...
        # Firing transactions with "blackfuryd tx bank send" in rapid succession does not work. This is currently a
        ...
        # separated by by comma: "blackfuryd tx bank send ... 100denoma,100denomb,100denomc") and wait for destination
        ...
        # self.blackfuryd_burn_gas_cost = 16 * 10**10 * 393000  # see x/ethbridge/types/msgs.go for gas
        # self.blackfuryd_lock_gas_cost = 16 * 10**10 * 393000
    ...
    def keys_delete(self, name: str):
        self.cmd.execst(["blackfuryd", "keys", "delete", name] + self._home_args() + self._keyring_backend_args(),
    ...
    def add_genesis_account(self, blackfuryadmin_addr: cosmos.Address, tokens: cosmos.Balance):
        tokens_str = cosmos.balance_format(tokens)
        self.blackfuryd_exec(["add-genesis-account", blackfuryadmin_addr, tokens_str] + self._home_args() + self._keyring_backend_args())
    ...
    def enable_rpc_port(self):
        ...
        app_toml["api"]["address"] = format_node_url(ANY_ADDR, BLACKFURYD_DEFAULT_API_PORT)
        self.save_app_toml(app_toml)

    def get_effective_home(self) -> str:
        return self.home if self.home is not None else self.cmd.get_user_home(".blackfuryd")
    ...
    def set_genesis_oracle_admin(self, address):
        self.blackfuryd_exec(["set-genesis-oracle-admin", address] + self._home_args() + self._keyring_backend_args())

    def set_genesis_token_registry_admin(self, address):
        self.blackfuryd_exec(["set-genesis-token-registry-admin", address] + self._home_args())

    def set_genesis_whitelister_admin(self, address):
        self.blackfuryd_exec(["set-genesis-whitelister-admin", address] + self._home_args() + self._keyring_backend_args())

    def set_gen_denom_whitelist(self, denom_whitelist_file):
        self.blackfuryd_exec(["set-gen-denom-whitelist", denom_whitelist_file] + self._home_args())
    ...
    # JSON file can be generated with "blackfuryd q tokenregistry generate"
    def create_tokenregistry_entry(self, symbol: str, offsideswap_symbol: str, decimals: int,
        ...
        return {
            ...
            "denom": offsideswap_symbol,
            "base_denom": offsideswap_symbol,
            "path": "",
            "ibc_channel_id": "",
    ...
    # from_black_addr has to be the address which was used at genesis time for "set-genesis-whitelister-admin".
    ...
    def token_registry_register(self, entry: TokenRegistryParams, from_black_addr: cosmos.Address,
        ...
        assert self.keys_show(from_black_addr)
        ...
        with self._with_temp_json_file(token_data) as tmp_registry_json:
            args = ["tx", "tokenregistry", "register", tmp_registry_json, "--from", from_black_addr, "--output",
            ...
            # Example of successful output: {"height":"196804","txhash":"C8252E77BCD441A005666A4F3D76C99BD35F9CB49AA1BE44CBE2FFCC6AD6ADF4","codespace":"","code":0,"data":"0A270A252F7369666E6F64652E746F6B656E72656769737472792E76312E4D73675265676973746572","raw_log":"[{\"events\":[{\"type\":\"message\",\"attributes\":[{\"key\":\"action\",\"value\":\"/blackfury.tokenregistry.v1.MsgRegister\"}]}]}]","logs":[{"msg_index":0,"log":"","events":[{"type":"message","attributes":[{"key":"action","value":"/blackfury.tokenregistry.v1.MsgRegister"}]}]}],"info":"","gas_wanted":"200000","gas_used":"115149","tx":null,"timestamp":""}
    ...
    def token_registry_register_batch(self, from_black_addr: cosmos.Address, entries: Iterable[TokenRegistryParams]):
        account_number, account_sequence = self.get_acct_seq(from_black_addr)
        token_registry_entries_before = set(e["denom"] for e in self.query_tokenregistry_entries())
        for entry in entries:
            res = self.token_registry_register(entry, from_black_addr, account_seq=(account_number, account_sequence))
            check_raw_log(res)
            account_sequence += 1
    ...
    def peggy2_token_registry_set_registry(self, registry_path: str, gas_prices: GasFees, gas_adjustment: float,
        ...
        args = ["tx", "tokenregistry", "set-registry", registry_path, "--gas-prices", black_format_amount(*gas_prices),
    ...
    def peggy2_set_cross_chain_fee(self, admin_account_address, network_id, ethereum_cross_chain_fee_token,
        ...
        args = ["tx", "ethbridge", "set-cross-chain-fee", str(network_id),
            ...
            str(cross_chain_burn_fee), "--from", admin_account_name, "--gas-prices", black_format_amount(*gas_prices),
            "--gas-adjustment", str(gas_adjustment)] + self._home_args() + self._keyring_backend_args() + \
            self._chain_id_args() + self._yes_args()
        return self.blackfuryd_exec(args)
    ...
    def blackfuryd_start(self, tcp_url: Optional[str] = None, minimum_gas_prices: Optional[GasFees] = None,
        ...
        blackfuryd_exec_args = self.build_start_cmd(tcp_url=tcp_url, p2p_laddr=p2p_laddr, grpc_address=grpc_address,
        ...
        return self.cmd.spawn_asynchronous_process(blackfuryd_exec_args, log_file=log_file)
    ...
    def build_start_cmd(self, tcp_url: Optional[str] = None, p2p_laddr: Optional[str] = None,
        ...
        args = [self.binary, "start"] + \
            (["--trace"] if trace else []) + \
            (["--minimum-gas-prices", black_format_amount(*minimum_gas_prices)] if minimum_gas_prices is not None else []) + \
    ...
    def send(self, from_black_addr: cosmos.Address, to_black_addr: cosmos.Address, amounts: cosmos.Balance,
        account_seq: Optional[Tuple[int, int]] = None, broadcast_mode: Optional[str] = None
    ) -> JsonDict:
        ...
        args = ["tx", "bank", "send", from_black_addr, to_black_addr, amounts_string, "--output", "json"] + \
    ...
    def send_and_check(self, from_addr: cosmos.Address, to_addr: cosmos.Address, amounts: cosmos.Balance
        ...
        expected_tx_fee = {FURY: black_tx_fee_in_fury}
    ...
    def get_balance(self, black_addr: cosmos.Address, height: Optional[int] = None,
        disable_log: bool = False, retries_on_error: Optional[int] = None, delay_on_error: int = 3
    ) -> cosmos.Balance:
        base_args = ["query", "bank", "balances", black_addr]
    ...
    def wait_for_balance_change(self, black_addr: cosmos.Address, old_balance: cosmos.Balance,
        ...
        log.debug("Waiting for balance to change for account {}...".format(black_addr))
        ...
        while True:
            new_balance = self.get_balance(black_addr, disable_log=disable_log)
            ...
            if (timeout is not None) and (timeout > 0) and (now - start_time > timeout):
                raise BlackfurydException("Timeout waiting for black account {} balance to change ({}s)".format(black_addr, timeout))
            if last_change_time is None:
                ...
                if (change_timeout is not None) and (change_timeout > 0) and (now - last_change_time > change_timeout):
                    raise BlackfurydException("Timeout waiting for black account {} balance to change ({}s)".format(black_addr, change_timeout))
    ...
    def _paged_read(self, base_args: List[str], result_key: str, height: Optional[int] = None,
        ...
        while True:
            ...
            while True:
                try:
                    res = self.blackfuryd_exec(args, disable_log=disable_log)
                    break
    ...
    # See https://github.com/Offsideswap/blackfury/blob/master/docs/tutorials/clp%20tutorial.md
    ...
    def clp_reward_period(self, from_addr: cosmos.Address, rewards_params: RewardsParams):
        with self._with_temp_json_file([rewards_params]) as tmp_rewards_json:
            args = ["tx", "clp", "reward-period", "--from", from_addr, "--path", tmp_rewards_json] + \
                ...
                self._fees_args() + self._broadcast_mode_args() + self._yes_args()
            res = self.blackfuryd_exec(args)
            res = blackfuryd_parse_output_lines(stdout(res))
            return res
    ...
    def sign_transaction(self, tx: JsonDict, from_black_addr: cosmos.Address, sequence: int = None,
        ...
        with self._with_temp_json_file(tx) as tmp_tx_file:
            args = ["tx", "sign", tmp_tx_file, "--from", from_black_addr] + \
    ...
    def encode_transaction(self, tx: JsonObj) -> bytes:
        with self._with_temp_json_file(tx) as tmp_file:
            res = self.blackfuryd_exec(["tx", "encode", tmp_file])
            encoded_tx = base64.b64decode(stdout(res))
            return encoded_tx

    def version(self) -> str:
        return exactly_one(stderr(self.blackfuryd_exec(["version"])).splitlines())
    ...
    def gov_submit_software_upgrade(self, version: str, from_acct: cosmos.Address, deposit: cosmos.Balance,
        ...
        args = ["tx", "gov", "submit-proposal", "software-upgrade", version, "--from", from_acct, "--deposit",
            ...
            self._broadcast_mode_args(broadcast_mode=broadcast_mode) + self._yes_args()
        res = yaml_load(stdout(self.blackfuryd_exec(args)))
        return res
    ...
    def query_gov_proposals(self) -> JsonObj:
        ...
        res = self.blackfuryd_exec(args, check_exit=False)
        if res[0] == 1:
    ...
    def blackfuryd_exec(self, args: List[str], stdin: Union[str, bytes, Sequence[str], None] = None,
    ...
    def _get_host_and_port(self) -> Tuple[str, int]:
        ...
        if self.node is None:
            return LOCALHOST, BLACKFURYD_DEFAULT_RPC_PORT
        else:
    ...
    def wait_for_last_transaction_to_be_mined(self, count: int = 1, disable_log: bool = True, timeout: int = 90):
        log.debug("Waiting for last blackfury transaction to be mined...")
        start_time = time.time()
        initial_block = self.get_current_block()
        while self.get_current_block() < initial_block + count:
            time.sleep(1)
            if time.time() - start_time > timeout:
                raise Exception("Timeout expired while waiting for last blackfury transaction to be mined")
    ...
    def _wait_up(self, timeout: int = 30):
        ...
        while True:
            try:
                ...
                pass
            if time.time() - start_time > timeout:
                raise BlackfurydException("Timeout waiting for blackfuryd to come up. Check if the process is running. "
    ...
    # However, this is needed for "blackfuryd tx bank send" which does not work with "--gas"
    def _fees_args(self) -> List[str]:
        return ["--fees", black_format_amount(self.fees, FURY)]
...
# Refactoring in progress - this class is supposed to become the successor of class Blackfury.
# It wraps node, home, chain_id, fees and keyring backend
# TODO Remove 'ctx' (currently needed for cross-chain fees for Peggy2)
class BlackfuryClient:
    def __init__(self, ctx, blackfury: Blackfuryd, node: Optional[str] = None, chain_id: Optional[str] = None,
        grpc_port: Optional[int] = None
    ):
        self.blackfury = blackfury
        self.ctx = ctx  # TODO Remove (currently needed for cross-chain fees for Peggy2)
        self.grpc_port = grpc_port

    def query_account(self, black_addr):
        result = json.loads(stdout(self.blackfury.blackfuryd_exec(["query", "account", black_addr, "--output", "json"])))
        return result

    def send_from_offsideswap_to_ethereum(self, from_black_addr: cosmos.Address, to_eth_addr: str, amount: int, denom: str,
        generate_only: bool = False
    ) -> Mapping:
        """ Sends ETH from Offsideswap to Ethereum (burn) """
        assert self.ctx.eth
        eth = self.ctx.eth
        ...
        if on_peggy2_branch:
            ...
            args = ["tx", "ethbridge", direction, to_eth_addr, str(amount), denom, str(cross_chain_ceth_fee),
                    ...
                    "--from", from_black_addr,  # Mandatory, either name from keyring or address
                    "--output", "json",
                ] + \
                (["--generate-only"] if generate_only else []) + \
                self.blackfury._fees_args() + \
                self.blackfury._home_args() + \
                (self.blackfury._keyring_backend_args() if not generate_only else []) + \
                self.blackfury._chain_id_args() + \
                self.blackfury._node_args() + \
                self.blackfury._yes_args()
            res = self.blackfury.blackfuryd_exec(args)
            ...
            args = ["tx", "ethbridge", direction] + \
                self.blackfury._node_args() + \
                [from_black_addr, to_eth_addr, str(amount), denom, cross_chain_ceth_fee] + \
                (self.blackfury._keyring_backend_args() if not generate_only else []) + \
                self.blackfury._fees_args() + \
                ["--ethereum-chain-id", ethereum_chain_id] + \
                self.blackfury._chain_id_args() + \
                self.blackfury._home_args() + \
                ["--from", from_black_addr] + \
                ["--output","json"] + \
                self.blackfury._yes_args()
            ...
            # blackfuryd tx ethbridge <direction> <node> <offsideswap_addr> <ethereum_addr> <amount> <symbol> <keyring backend> <ethereum-chain-id>


    def send_from_offsideswap_to_ethereum_grpc(self, from_black_addr: cosmos.Address, to_eth_addr: str, amount: int,
        denom: str
    ):
        tx = self.send_from_offsideswap_to_ethereum(from_black_addr, to_eth_addr, amount, denom, generate_only=True)
        signed_tx = self.blackfury.sign_transaction(tx, from_black_addr)
        encoded_tx = self.blackfury.encode_transaction(signed_tx)
    ...
    def open_grpc_channel(self) -> grpc.Channel:
        ...
        # See https://app.swaggerhub.com/apis/Ivan-Verchenko/blackfury-swagger-api/1.1.1
        # See https://raw.githubusercontent.com/Offsideswap/offsideswap-ui/develop/ui/core/swagger.yaml
        return grpc.insecure_channel("{}:{}".format(LOCALHOST,
            self.grpc_port if self.grpc_port is not None else BLACKFURYD_DEFAULT_GRPC_PORT))
...
class Blackgen:
    def __init__(self, cmd):
        self.cmd = cmd
        self.binary = "blackgen"

    # Reference: docker/localnet/blackfury/root/scripts/blackfury.sh (branch future/peggy2):
    ...
    def create_standalone(self, chainnet, moniker, mnemonic, bind_ip_address, keyring_backend=None):
        ...
        return self.blackgen_exec(args, keyring_backend=keyring_backend)

    def blackgen_exec(self, args, keyring_backend=None, cwd=None, env=None):
        args = [self.binary] + args + \
...
class Ebrelayer:
    ...
    def init(self, tendermind_node, web3_provider, bridge_registry_contract_address, validator_moniker,
        ...
        args = [self.binary, "init", tendermind_node, web3_provider, bridge_registry_contract_address,
            ...
            (["--gas-prices", black_format_amount(*gas_prices)] if gas_prices is not None else []) + \
...
class BlackfurydException(Exception):
...
def is_min_commission_too_low_exception(e: Exception):
    ...
    return (type(e) == BlackfurydException) and patt.match(e.message)
...
class RateLimiter:
    def __init__(self, blackfuryd, max_tpb):
        self.blackfuryd = blackfuryd
    ...
    def limit(self):
        ...
        if self.counter == self.max_tpb:
            self.blackfuryd.wait_for_last_transaction_to_be_mined()
            self.counter = 0
