...
import blacktool.eth
from blacktool import eth, cosmos, command
from blacktool.hardhat import Hardhat, default_accounts as hardhat_default_accounts
from blacktool.geth import Geth
from blacktool.truffle import Ganache
from blacktool.localnet import Localnet
from blacktool.command import Command
from blacktool.offsideswap import Blackgen, Blackfuryd, Ebrelayer, offsideswap_denom_hash, FURY
from blacktool.project import Project, killall, force_kill_processes
...
class Integrator(Ganache, Command):
    ...
    # Peggy1 only
    def offsideswap_init_integration(self, blackfury, validator_moniker, validator_mnemonic, denom_whitelist_file):
        # now we have to add the validator key to the test keyring so the tests can send fury from validator1
        blackfury0 = Blackfuryd(self)
        blackfury0.keys_add(validator_moniker, validator_mnemonic)
        valoper = blackfury.keys_show(validator_moniker, bech="val")[0]["address"]
        assert valoper == blackfury0.get_val_address(validator_moniker)  # This does not use "home"; if it the assertion holds it could be grouped with offsideswap_init_peggy

        # This was deleted in commit f00242302dd226bc9c3060fb78b3de771e3ff429 from offsideswap_start_daemon.sh because
        # it was not working. But we assume that we want to keep it.
        blackfury.blackfuryd_exec(["add-genesis-validators", valoper] + blackfury._home_args())

        # Add blackfuryadmin to ~/.blackfuryd
        blackfury0 = Blackfuryd(self)
        adminuser_addr = blackfury0.keys_add("blackfuryadmin")["address"]
        tokens = {FURY: 10 ** 28}
        # Original from peggy:
        # self.cmd.execst(["blackfuryd", "add-genesis-account", blackfuryd_admin_address, "100000000000000000000fury", "--home", blackfuryd_home])
        blackfury.add_genesis_account(adminuser_addr, tokens)
        blackfury.set_genesis_oracle_admin(adminuser_addr)
        blackfury.set_gen_denom_whitelist(denom_whitelist_file)

        return adminuser_addr

    def blackfuryd_peggy2_init_validator(self, blackfury, validator_moniker, validator_mnemonic, evm_network_descriptor, validator_power):
        # Add validator key to test keyring
        # This effectively copies key for validator_moniker from what blackgen creates in /tmp/blackfurydNetwork/validators
        # to ~/.blackfuryd (note absence of explicit blackfuryd_home, therefore it's ~/.blackfuryd)
        blackfury0 = Blackfuryd(self)
        ...
        valoper = blackfury.get_val_address(validator_moniker)

        # Add genesis validator
        blackfury.add_genesis_validators_peggy(evm_network_descriptor, valoper, validator_power)

    # @TODO Move to Blackgen class
    def blackgen_create_network(self, chain_id: str, validator_count: int, networks_dir: str, network_definition_file: str,
        seed_ip_address: str, mint_amount: Optional[cosmos.Balance] = None
    ):
        # Old call (no longer works either):
        # blackgen network create localnet 1 /mnt/shared/work/projects/black/blackfury/local-tmp/my/deploy/rake/../networks \
        #     192.168.1.2 /mnt/shared/work/projects/black/blackfury/local-tmp/my/deploy/rake/../networks/network-definition.yml \
        #     --keyring-backend file
        # self.cmd.execst(["blackgen", "network", "create", "localnet", str(validator_count), networks_dir, seed_ip_address,
        ...
        args = ["blackgen", "network", "create", chain_id, str(validator_count), networks_dir, seed_ip_address,
            network_definition_file, "--keyring-backend", "test"] + \
            (["--mint-amount", cosmos.balance_format(mint_amount)] if mint_amount else [])
        self.execst(args)

    # def wait_for_black_account(self, netdef_json, validator1_address):
    #     # TODO Replace with test_utilities.wait_for_black_account / wait_for_black_account_up
    #     return self.execst(["python3", os.path.join(self.project.test_integration_dir, "src/py/wait_for_black_account.py"),
    #         netdef_json, validator1_address], env={"USER1ADDR": "nothing"})

    def wait_for_black_account_up(self, address: cosmos.Address, tcp_url: str = None, timeout: int = 90):
        # TODO Move to Blackfuryd class
        ...
        #     time.sleep(1)
        # self.wait_for_black_account(netdef_json, validator1_address)
        ...
        args = ["blackfuryd", "query", "account", address] + \
        ...
        while True:
            try:
                ...
                if last_exception is None:
                    log.debug(f"Waiting for black account {address}...")
                if time.time() - start_time > timeout:
                    message = "Timeout expired while waiting for black account {} to become available".format(address)
...
class UIStackEnvironment:
    def __init__(self, cmd):
        ...
        self.chain_id = "offsideswap-local"
        ...
        self.blackfuryd_path = self.cmd.get_user_home(".blackfuryd")
        self.blackfury = Blackfuryd(cmd)
    ...
    def stack_save_snapshot(self):
        ...
        # ui/scripts/stack-launch.sh -> ui/scripts/_black-build.sh -> ui/chains/black/build.sh
        # killall blackfuryd
        # rm $(which blackfuryd)
        self.cmd.rmdir(self.blackfuryd_path)
        ...
        blackfury = Blackfuryd(self.cmd)
        # ui/scripts/stack-launch.sh -> ui/scripts/_black.sh -> ui/chains/black/launch.sh
        blackfury.blackfuryd_init("test", self.chain_id)
        self.cmd.copy_file(project_dir("ui/chains/black/app.toml"), os.path.join(self.blackfuryd_path, "config/app.toml"))
        log.info(f"Generating deterministic account - {self.shadowfiend_name}...")
        shadowfiend_account = self.cmd.blackfuryd_keys_add(self.shadowfiend_name, self.shadowfiend_mnemonic)
        log.info(f"Generating deterministic account - {self.akasha_name}...")
        akasha_account = self.blackfury.keys_add(self.akasha_name, self.akasha_mnemonic)
        log.info(f"Generating deterministic account - {self.juniper_name}...")
        juniper_account = self.cmd.blackfuryd_keys_add(self.juniper_name, self.juniper_mnemonic)
        ...
        assert shadowfiend_address == self.blackfury.keys_show(self.shadowfiend_name)[0]["address"]
        assert akasha_address == self.blackfury.keys_show(self.akasha_name)[0]["address"]
        assert juniper_address == self.blackfury.keys_show(self.juniper_name)[0]["address"]
        ...
        blackfury.add_genesis_account(shadowfiend_address, tokens_shadowfiend)
        blackfury.add_genesis_account(akasha_address, tokens_akasha)
        blackfury.add_genesis_account(juniper_address, tokens_juniper)

        shadowfiend_address_bech_val = blackfury.keys_show(self.shadowfiend_name, bech="val")[0]["address"]
        self.cmd.blackfuryd_add_genesis_validators(shadowfiend_address_bech_val)

        amount = black_format_amount(10**24, "stake")
        self.cmd.execst(["blackfuryd", "gentx", self.shadowfiend_name, amount, f"--chain-id={self.chain_id}",
            f"--keyring-backend={self.keyring_backend}"])

        log.info("Collecting genesis txs...")
        self.cmd.execst(["blackfuryd", "collect-gentxs"])
        log.info("Validating genesis file...")
        self.cmd.execst(["blackfuryd", "validate-genesis"])

        log.info("Starting test chain...")
        blackfuryd_proc = self.cmd.blackfuryd_start(minimum_gas_prices=(0.5, FURY))  # TODO blackfuryd_home=???

        # blackfuryd must be up before continuing
        self.cmd.black_wait_up("localhost", 1317)
        ...
        # ui/scripts/_migrate.sh -> ui/chains/black/migrate.sh
        # Original scripts say "if we don't sleep there are issues"
        time.sleep(10)
        log.info("Creating liquidity pool from catk:fury...")
        blackfury.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "catk", [10**5, FURY], 10**25, 10**25)
        time.sleep(5)
        log.info("Creating liquidity pool from cbtk:fury...")
        blackfury.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "cbtk", [10**5, FURY], 10**25, 10**25)
        # should now be able to swap from catk:cbtk
        time.sleep(5)
        log.info("Creating liquidity pool from ceth:fury...")
        blackfury.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "ceth", [10**5, FURY], 10**25, 83*10**20)
        # should now be able to swap from x:ceth
        time.sleep(5)
        log.info("Creating liquidity pool from cusdc:fury...")
        blackfury.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "cusdc", [10**5, FURY], 10**25, 10**25)
        time.sleep(5)
        log.info("Creating liquidity pool from clink:fury...")
        blackfury.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "clink", [10**5, FURY], 10**25, 588235*10**18)
        time.sleep(5)
        log.info("Creating liquidity pool from ctest:fury...")
        blackfury.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "ctest", [10**5, FURY], 10**25, 10**13)
        ...
        # At this point we have 3 running processes - ganache_proc, blackfuryd_proc and ebrelayer_proc
        # await black-node-up and migrate-complete
        ...
        # ui/chains/black/snapshot.sh:
        self.cmd.tar_create(self.blackfuryd_path, os.path.join(snapshots_dir, "black.tar.gz"))
    ...
    def stack_push(self):
        ...
        image_root = "ghcr.io/offsideswap/blackfury/ui-stack"
...
class IntegrationTestsEnvironment:
    def __init__(self, cmd):
        ...
        # set_persistant_env_var CONTAINER_NAME integration_blackfury1_1 $envexportfile
        ...
        self.network_name = "develop"
        self.network_id = 5777
        self.peruser_storage_dir = self.cmd.get_user_home(".blackfury-integration")
        self.state_vars = {}
    ...
    def run(self):
        ...
        log_dir = self.cmd.tmpdir("blackfury")
        self.cmd.mkdir(log_dir)
        ganache_log_file = open(os.path.join(log_dir, "ganache.log"), "w")  # TODO close
        blackfuryd_log_file = open(os.path.join(log_dir, "blackfuryd.log"), "w")  # TODO close
        ...
        # # TODO This should be last (after return from setup_offsideswap.sh)
        # burn_limits = [
        #     [eth.NULL_ADDRESS, 31*10**18],
        ...
        # test/integration/setup_offsideswap.sh:
        ...
        # New:
        # blackgen network create localnet 1 $NETWORKDIR 192.168.1.2 $NETWORKDIR/network-definition.yml --keyring-backend test \
        ...
        self.cmd.blackgen_create_network(chain_id, validator_count, networks_dir, network_definition_file, seed_ip_address, mint_amount=self.mint_amount)
        ...
        blackfuryd_home = os.path.join(chaindir, ".blackfuryd")
        denom_whitelist_file = os.path.join(self.test_integration_dir, "whitelisted-denoms.json")
        # BLACKFURYD_LOG=$datadir/logs/blackfuryd.log

        blackfury = Blackfuryd(self.cmd, home=blackfuryd_home)

        adminuser_addr = self.cmd.offsideswap_init_integration(blackfury, validator1_moniker, validator1_mnemonic,
            denom_whitelist_file)

        # Start blackfuryd
        blackfuryd_proc = blackfury.blackfuryd_start(tcp_url=self.tcp_url, minimum_gas_prices=(0.5, FURY),
            log_file=blackfuryd_log_file, trace=True)

        # TODO: wait for blackfuryd to come up before continuing
        # in offsideswap_start_daemon.sh: "sleep 10"
        # in offsideswap_run_ebrelayer.sh (also run_ebrelayer here) we already wait for connection to port 26657 and black account validator1_addr

        # Removed
        # # TODO Process exits immediately with returncode 1
        # # TODO Why does it not stop start-integration-env.sh?
        # # rest_server_proc = self.cmd.popen(["blackfuryd", "rest-server", "--laddr", "tcp://0.0.0.0:1317"])  # TODO cwd

        # test/integration/offsideswap_start_ebrelayer.sh -> test/integration/offsideswap_run_ebrelayer.sh
        # This script is also called from tests

        relayer_db_path = os.path.join(self.test_integration_dir, "offsideswaprelayerdb")
        ...
        self.state_vars = {
            ...
            "EBRELAYER_ETHEREUM_PRIVATE_KEY": ebrelayer_ethereum_private_key,  # Needed by offsideswap_run_ebrelayer.sh
            ...
            "OFFSIDESWAP_ADMIN_ACCOUNT": adminuser_addr,  # Needed by test_peggy_fees.py (via conftest.py)
            "EBRELAYER_DB": relayer_db_path,  # Created by offsideswap_run_ebrelayer.sh, does not appear to be used anywhere at the moment
        }
        ...
        from blacktool import localnet
        localnet.run_localnet_hook()

        return ganache_proc, blackfuryd_proc, ebrelayer_proc

    def remove_and_add_blackfuryd_keys(self, moniker, mnemonic):
        ...
        # res = self.cmd.execst(["blackfuryd", "keys", "delete", moniker, "--keyring-backend", "test"], stdin=["y"])
        blackfury = Blackfuryd(self.cmd)
        blackfury.keys_delete(moniker)
        blackfury.keys_add(moniker, mnemonic)
    ...
    def run_ebrelayer(self, netdef_json, validator1_address, validator_moniker, validator_mnemonic,
        ...
        while not self.cmd.tcp_probe_connect("localhost", 26657):
            time.sleep(1)
        # self.cmd.wait_for_black_account(netdef_json, validator1_address)
        self.cmd.wait_for_black_account_up(validator1_address)
        time.sleep(10)
        self.remove_and_add_blackfuryd_keys(validator_moniker, validator_mnemonic)  # Creates ~/.blackfuryd/keyring-tests/xxxx.address
    ...
    def create_snapshot(self, snapshot_name):
        ...
        self.cmd.tar_create(self.state_vars["EBRELAYER_DB"], os.path.join(named_snapshot_dir, "offsideswaprelayerdb.tar.gz"))
        ...
        self.cmd.tar_create(self.cmd.get_user_home(".blackfuryd"), os.path.join(named_snapshot_dir, "blackfuryd.tar.gz"))
    ...
    def restore_snapshot(self, snapshot_name):
        ...
        assert os.path.realpath(relayer_db_path) == os.path.realpath(os.path.join(self.test_integration_dir, "offsideswaprelayerdb"))
        extract("ganache.tar.gz", ganache_db_dir)
        extract("offsideswaprelayerdb.tar.gz", relayer_db_path)
        ...
        extract("blackfuryd.tar.gz", self.cmd.get_user_home(".blackfuryd"))  # Needed for "--keyring-backend test"
    ...
    def restart_processes(self):
        ...
        blackfuryd_home = os.path.join(chaindir, ".blackfuryd")
        blackfuryd_proc = self.cmd.blackfuryd_start(tcp_url=self.tcp_url, minimum_gas_prices=(0.5, FURY),
            blackfuryd_home=blackfuryd_home, trace=True)
...
# Peggy2 environment
# Originally derived from devenv which ran hardhat, blackfuryd, ebrelayer like this: cd smart-contracts; GOBIN=~/go/bin npx hardhat run scripts/devenv.ts
class Peggy2Environment(IntegrationTestsEnvironment):
    def __init__(self, cmd: Command, witness_count: int = 1, witness_power: int = 100, consensus_threshold: int = 49,
        ethereum_ws_port: int = 8545, ethereum_chain_id: int = 9999, black_chain_id = "localnet", use_geth: bool = False
    ):
        super().__init__(cmd)
        ...
        self.chain_id = black_chain_id
        self.ceth_symbol = offsideswap_denom_hash(self.ethereum_chain_id, eth.NULL_ADDRESS)
        assert self.ceth_symbol == "blackBridge99990x0000000000000000000000000000000000000000"

        # This goes to validator0, i.e. blackfury_validators[0]["address"]
        self.validator_mint_amounts: cosmos.Balance = {
            FURY: 999999 * 10**27,
            self.ceth_symbol: 999999 * 10**21,
            "blackBridge00030x1111111111111111111111111111111111111111": 137 * 10**16,
            "ibc/FEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACE": 137 * 10**16,
        }
        ...
        self.admin_account_mint_amounts: cosmos.Balance = {
            ...
            "blackBridge00030x1111111111111111111111111111111111111111": 10 ** 16,
    ...
    def run(self) -> Tuple[subprocess.Popen, subprocess.Popen, subprocess.Popen, List[subprocess.Popen]]:
        ...
        # - build_golang_binaries before start_offsideswap
        # - start_hardhat before deploy_smart_contracts
        # - start_offsideswap before start_witnesses_and_relayers
        ...
        self.cmd.rmdir(self.cmd.get_user_home(".blackfuryd"))  # Purge test keyring backend
        ...
        admin_account_name = "blackfuryadmin"
        ...
        blackfuryd_network_dir = self.cmd.tmpdir("blackfurydNetwork")  # Gets written to .vscode/launch.json
        self.cmd.rmdir(blackfuryd_network_dir)
        self.cmd.mkdir(blackfuryd_network_dir)
        network_config_file, blackfuryd_exec_args, blackfuryd_proc, tcp_url, admin_account_address, blackfury_validators, \
            blackfury_relayers, blackfury_witnesses, blackfury_validator0_home, chain_dir = \
                self.init_offsideswap(blackfuryd_network_dir, blackfuryd_log_file, self.validator_mint_amounts,
        ...
        log.debug("Validator 0 address: {}".format(blackfury_validators[0]["address"]))  # mint
        ...
        [relayer0_exec_args], [witness_exec_args] = self.start_witnesses_and_relayers(
            ...
            blackfury_validators, blackfury_relayers, blackfury_witnesses, symbol_translator_file, self.log_level_relayer,
            self.log_level_witness, relayer_extra_args)

        hardhat_scripts.update_validator_power(peggy_sc_addrs["CosmosBridge"], evm_validator_addresses, blackfury_witnesses)
        ...
        _unused_peggy2_environment = {
            "admin": {
                ...
                "home": blackfury_validator0_home,
            },
            "symbol_translator_file": symbol_translator_file,
            "w3_url": w3_url,
            "evm_chain_id": self.ethereum_chain_id,
            "chain_id": self.chain_id,
            "validators": blackfury_validators,  # From yaml file generated by blackgen
            "relayers": blackfury_relayers,
        ...
        self.write_env_files(self.project.project_dir(), self.project.go_bin_dir, peggy_sc_addrs, hardhat_accounts,
            admin_account_name, admin_account_address, blackfury_validator0_home, blackfury_validators, blackfury_relayers,
            blackfury_witnesses, tcp_url, hardhat_bind_hostname, hardhat_port, self.ethereum_chain_id, chain_dir,
            blackfuryd_exec_args, relayer0_exec_args, witness_exec_args
        )

        return hardhat_proc, blackfuryd_proc, relayer0_proc, witness_procs

    def init_offsideswap(self, blackfuryd_network_dir: str, blackfuryd_log_file: TextIO,
        ...
        try:
            self.cmd.blackgen_create_network(self.chain_id, validator_count, blackfuryd_network_dir, network_config_file_path,
        ...
        chain_dir_base = os.path.join(blackfuryd_network_dir, "validators", self.chain_id)
        ...
        for validator in validators:
            ...
            blackfuryd_home = os.path.join(chain_dir_base, validator_moniker, ".blackfuryd")
            blackfury = Blackfuryd(self.cmd, home=blackfuryd_home)
            self.cmd.blackfuryd_peggy2_init_validator(blackfury, validator_moniker, validator_mnemonic, evm_network_descriptor, validator_power)

        # TODO Needs to be fixed when we support more than 1 validator
        validator0 = exactly_one(validators)
        validator0_home = os.path.join(chain_dir_base, validator0["moniker"], ".blackfuryd")
        validator0_address = validator0["address"]
        chain_dir = os.path.join(chain_dir_base, validator0["moniker"])

        blackfury = Blackfuryd(self.cmd, home=validator0_home, chain_id=self.chain_id)

        # Create an ADMIN account on blackfury with name admin_account_name (e.g. "blackfuryadmin")
        admin_account_address = blackfury.peggy2_add_account(admin_account_name, admin_account_mint_amounts, is_admin=True)

        if self.extra_balances_for_admin_account:
            blackfury.add_genesis_account_directly_to_existing_genesis_json({admin_account_address: self.extra_balances_for_admin_account})

        # TODO Check if blackfuryd_peggy2_add_relayer_witness_account can be executed offline (without blackfuryd running)
        # TODO Check if blackfuryd_peggy2_set_cross_chain_fee can be executed offline (without blackfuryd running)

        # Create an account for each relayer
        # Note: "--home" is shared with blackfuryd's "--home"
        relayers = [{
            "name": name,
            "address": blackfury.peggy2_add_relayer_witness_account(name, admin_account_mint_amounts,
        ...
        gas_prices = (0.5, FURY)
        # @TODO Detect if blackfuryd is already running, for now it fails silently and we wait forever in wait_for_black_account_up
        blackfuryd_exec_args = blackfury.build_start_cmd(tcp_url=tcp_url, minimum_gas_prices=gas_prices,
            log_format_json=True, log_level="debug")
        blackfuryd_proc = self.cmd.spawn_asynchronous_process(blackfuryd_exec_args, log_file=blackfuryd_log_file)

        time_before = time.time()
        self.cmd.wait_for_black_account_up(validator0_address, tcp_url)
        log.debug("Time for blackfuryd to come up: {:.2f}s".format(time.time() - time_before))
        ...
        res = blackfury.peggy2_token_registry_set_registry(registry_json, gas_prices, 1.5, admin_account_address,
            self.chain_id)
        ...
        blackfury.wait_for_last_transaction_to_be_mined()
        ...
        gas_prices = [0.5, FURY]
        gas_adjustment = 1.5
        blackfury.peggy2_set_cross_chain_fee(admin_account_address, self.ethereum_chain_id,
        ...
        # signatures. This delay waits for block production. (See commit 5854d8b6f3970c1254cac0eca0e3817354151853)
        blackfury.wait_for_last_transaction_to_be_mined()
        blackfury.peggy2_update_consensus_needed(admin_account_address, self.ethereum_chain_id, self.consensus_threshold)

        return network_config_file, blackfuryd_exec_args, blackfuryd_proc, tcp_url, admin_account_address, validators, \
    ...
    def start_witnesses_and_relayers(self, web3_websocket_address: str, hardhat_chain_id: int, tcp_url: str,
        ...
        blackfury_validators: Sequence[Mapping[str, Any]], blackfury_relayers: Sequence[Mapping[str, Any]],
        blackfury_witnesses: Sequence[Mapping[str, Any]], symbol_translator_file: str, log_level_relayer: Optional[str],
        ...
        blackfury_validator0 = exactly_one(blackfury_validators)
        blackfury_validator0_address = blackfury_validator0["address"]

        blackfury_relayer0 = exactly_one(blackfury_relayers)
        blackfury_relayer0_mnemonic = blackfury_relayer0["name"]
        blackfury_relayer0_address = blackfury_relayer0["address"]
        blackfury_relayer0_home = blackfury_relayer0["home"]

        bridge_registry_contract_addr = peggy_sc_addrs["BridgeRegistry"]

        self.cmd.wait_for_black_account_up(blackfury_validator0_address, tcp_url=tcp_url)  # Required for both relayer and witness
        ...
        relayer0_exec_args = ebrelayer.peggy2_build_ebrelayer_cmd(
            ...
            blackfury_relayer0_mnemonic,
            chain_id=chain_id,
            node=tcp_url,
            sign_with=blackfury_relayer0_address,
            symbol_translator_file=symbol_translator_file,
            ethereum_address=evm_validator0_addr,
            ethereum_private_key=evm_validator0_key,
            keyring_backend="test",
            keyring_dir=blackfury_relayer0_home,
            home=blackfury_relayer0_home,
            log_level=log_level_relayer,
            **relayer_extra_args,
        )

        witness_exec_args = []
        for i, w in enumerate(blackfury_witnesses):
    ...
    def write_env_files(self, project_dir, go_bin_dir, evm_smart_contract_addrs, eth_accounts, admin_account_name,
        admin_account_address, blackfury_validator0_home, blackfury_validators, blackfury_relayers, blackfury_witnesses,
        tcp_url, hardhat_bind_hostname, hardhat_port, hardhat_chain_id, chain_dir, blackfuryd_exec_args,
        ...
        def format_black_account(black_account):
            return {"account": black_account["address"], "name": black_account["name"], "homeDir": black_account["home"]}
        ...
        environment_json = {
            ...
            "blackResults": {
                "validatorValues": blackfury_validators,
                "adminAddress": format_black_account({"address": admin_account_address, "name": admin_account_name, "home": blackfury_validator0_home}),
                "relayerAddresses": [format_black_account(a) for a in blackfury_relayers],
                "witnessAddresses": [format_black_account(a) for a in blackfury_witnesses],
                # "process": { ... },
                "tcpurl": tcp_url,
            }
        }
        ...
        dot_env = dict_merge({
            ...
            "GOBIN": go_bin_dir,
            "TCP_URL": tcp_url,
            "VALIDATOR_ADDRESS": blackfury_validators[0]["address"],
            "VALIDATOR_CONSENSUS_ADDRESS": blackfury_validators[0]["validator_consensus_address"],
            "VALIDATOR_MENOMONIC": blackfury_validators[0]["mnemonic"],
            "VALIDATOR_MONIKER": blackfury_validators[0]["moniker"],
            "VALIDATOR_PASSWORD": blackfury_validators[0]["password"],
            "VALIDATOR_PUB_KEY": blackfury_validators[0]["pub_key"],
        ...
        } for i, validator in enumerate(blackfury_validators)], *[{
            f"RELAYER_ADDRESS_{i}": relayer["address"],
            f"RELAYER_NAME_{i}": relayer["name"],
        } for i, relayer in enumerate(blackfury_relayers)], *[{
            f"WITNESS_ADDRESS_{i}": witness["address"],
            f"WITNESS_NAME_{i}": witness["name"],
        } for i, witness in enumerate(blackfury_witnesses)])
        ...
        launch_json = {
            ...
            "configurations": [
                {
                    "name": "Python: blacktool",
                    "type": "python",
                    "request": "launch",
                    "program": "${workspaceFolder}/test/integration/framework/blacktool",
                    "args": ["run-env"],
                    ...
                    "name": "Debug Blackfuryd",
                    "type": "go",
                    "request": "launch",
                    "mode": "debug",
                    "program": "cmd/blackfuryd",
                    # Generally we want to use blackfuryd_exec_args, except for:
                    # - here we don't have the initial "blackfuryd"
        ...
        for config in launch_json["configurations"]:
            if config["name"].startswith("Debug Relayer"):
                intellij_ebrelayer_configs.append(render_intellij_run_xml(
                    "ebrelayer devenv",
                    " ".join(config["args"]),
                    "github.com/Offsideswap/blackfury/cmd/ebrelayer",
                ...
                intellij_witness_configs.append(render_intellij_run_xml(
                    ...
                    "$PROJECT_DIR$/cmd/ebrelayer/main.go",
                    {"ETHEREUM_PRIVATE_KEY": dot_env["ETHEREUM_PRIVATE_KEY"]}))
            elif config["name"].startswith("Debug Blackfuryd"):
                intellij_blackfuryd_configs.append(render_intellij_run_xml(
                    "blackfuryd devenv",
                    " ".join(config["args"]),
                    "github.com/Offsideswap/blackfury/cmd/blackfuryd",
                    "$PROJECT_DIR$/cmd/blackfuryd/main.go",
                    {}))

        intellij_ebrelayer_config = exactly_one(intellij_ebrelayer_configs)
        intellij_witness_config = intellij_witness_configs[0]
        intellij_blackfuryd_config = exactly_one(intellij_blackfuryd_configs)
        ...
        self.cmd.write_text_file(os.path.join(run_dir, "blackfuryd.run.xml"), joinlines(intellij_blackfuryd_config))

        return environment_json, dot_env, launch_json, intellij_ebrelayer_config, intellij_witness_config, intellij_blackfuryd_config
...
class IBCEnvironment(IntegrationTestsEnvironment):
    ...
    def run(self):
        ...
        # Mnemonics can be generated with "blackgen key generate" or "blackfuryd keys mnemonic", but that gives us 24 words
        ...
        blackgen = Blackgen(self.cmd)
        # This does not work - "--keyring-backend" is not supported
        x = blackgen.create_standalone(chainnet0, "chain1", mnemonic, ipaddr0, keyring_backend=None)
