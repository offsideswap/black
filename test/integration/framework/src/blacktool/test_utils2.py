...
from blacktool import eth, test_utils, cosmos, offsideswap, command
from blacktool.common import *
from blacktool.offsideswap import FURY


def get_black_tx_fees(ctx):
    return {FURY: offsideswap.black_tx_fee_in_fury}


def get_black_burn_fees(ctx):
    return {FURY: offsideswap.black_tx_burn_fee_in_fury, ctx.ceth_symbol: offsideswap.black_tx_burn_fee_in_ceth}


# TODO Obsolete
def send_from_offsideswap_to_offsideswap(ctx: test_utils.EnvCtx, from_addr: cosmos.Address, to_addr: cosmos.Address,
    amounts: cosmos.Balance
) -> cosmos.Balance:
    return ctx.blackfury.send_and_check(from_addr, to_addr, amounts)

def send_erc20_from_offsideswap_to_ethereum(ctx: test_utils.EnvCtx, from_addr: cosmos.Address, to_addr: eth.Address,
    amount: int, denom: str
):
    token_address = get_erc20_token_address(ctx, denom)
    black_balance_before = ctx.get_offsideswap_balance(from_addr)
    eth_balance_before = ctx.get_erc20_token_balance(token_address, to_addr)
    ctx.blackfury_client.send_from_offsideswap_to_ethereum(from_addr, to_addr, amount, denom)
    ctx.wait_for_eth_balance_change(to_addr, eth_balance_before, token_addr=token_address)
    black_balance_after = ctx.get_offsideswap_balance(from_addr)
...
def get_erc20_token_address(ctx: test_utils.EnvCtx, black_denom_hash: str) -> eth.Address:
    ...
    ethereum_network_descriptor, token_address = offsideswap.offsideswap_denom_hash_to_token_contract_address(black_denom_hash)
...
class PredefinedWallets:
    ...
    def create(cmd: command.Command, count: int, path: str):
        blackfuryd = offsideswap.Blackfuryd(cmd, home=path)
        entries = []
