...
# Test environment for testing the new Offsideswap public SDK: https://docs.google.com/document/d/1MAlg-I0xMnUvbavAZdAN---WuqbyuRyKw-6Lfgfe130/edit
# IBC localnet tool: https://github.com/Offsideswap/offsideswap-deploy/commit/0424d2ec81f6f739a3a2cbc77feba1a6f297430d
...
chains = {
    ...
    "offsideswap": {"binary": "blackfuryd", "relayer": "ibc"},
}
...
configs = {
    "local": {
        ...
        "offsideswap": "http://127.0.0.1:26657",
...
# Generates a blackfuryd key and stores it into test keyring. Returns the mnemonic that can be used to
# recreate it.
...
def run_tests_for_one_chain_in_one_direction(config, other_chain, direction_flag, number_of_iterations):
    # Assume we're always sending transactions between other_chain and offsideswap.
    # from_chain is chain sending the assets, to_chain is the receiving chain.
    from_chain = "offsideswap" if direction_flag else other_chain
    to_chain = other_chain if direction_flag else "offsideswap"
    ...
    fees = config["fees"]
    gas = config["gas"]
    offsideswap_proc = start_chain("offsideswap")
    ...
    mnemonic = add_new_key_to_keyring("offsideswap", from_account)
