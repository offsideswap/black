...
from blacktool import test_utils, run_env, cosmos, diagnostics, offsideswap, frontend, test_utils2
from blacktool.run_env import Integrator, UIStackEnvironment, Peggy2Environment, IBCEnvironment, IntegrationTestsEnvironment
...
def main(argv):
    ...
    if what == "venv":
        ...
        if class_to_use == Peggy2Environment:
            ...
            hardhat_proc, blackfuryd_proc, relayer0_proc, witness_procs = env.run()
            processes = [hardhat_proc, blackfuryd_proc, relayer0_proc] + witness_procs
            ...
            processes = env.run()
            # TODO Cleanup:
            # - rm -rf test/integration/blackfuryrelayerdb
            # - rm -rf networks/validators/localnet/$moniker/.blackfuryd
        ...
        import blacktool.geth, blacktool.eth
        geth = blacktool.geth.Geth(cmd)
        datadir = os.path.join(os.environ["HOME"], ".blacktool-geth")
        datadir = None
        signer_addr, signer_private_key = blacktool.eth.web3_create_account()
        ...
        blackfuryd = offsideswap.Blackfuryd(cmd, node=args.node)
        from_block = args.from_block if args.from_block is not None else 1
        to_block = args.to_block if args.to_block is not None else blackfuryd.get_current_block()
        block_times = diagnostics.get_block_times(blackfuryd, from_block, to_block)
