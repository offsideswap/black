import os
import json
from blacktool.command import Command, ExecResult
...
def force_kill_processes(cmd):
    ...
    cmd.execst(["pkill", "blackfuryd"], check_exit=False)

def killall(processes):
    # TODO Order - ebrelayer, blackfuryd, ganache
    for p in processes:
        if p is not None:
            p.kill()
            p.wait()
...
class Project:
    ...
    def __init__(self, cmd: Command, base_dir: str):
        ...
        self.blacktool_dir = project_dir("test", "integration", "framework")
    ...
    def __rm_files(self, level):
        if level >= 0:
            ...
            self.__rm(self.cmd.get_user_home(".blackfuryd"))  # Probably needed for "--keyring-backend test"
            ...
            # rmdir ~/.cache/yarn
        if level >= 1:
            for file in ["blackfuryd", "ebrelayer", "blackgen"]:
        ...
        if level >= 2:
            ...
            self.__rm_run_env_files()
            self.__rm(self.cmd.get_user_home(".blackfury-integration"))
    ...
    def make_go_binaries(self):
        ...
        # (checks all *.go files and, runs make in $BASEDIR, touches blackfuryd, removes ~/.blackfuryd/localnet
        self.cmd.execst(["make"], cwd=project_dir("test", "integration"), env={"BASEDIR": project_dir()}, pipe=False)

    # From PeggyEnvironment
    # TODO Merge
    # Main Makefile requires GOBIN to be set to an absolute path. Compiled executables ebrelayer, blackgen and
    # blackfuryd will be written there. The directory will be created if it doesn't exist yet.
    ...
    def get_peruser_config_dir(self):
        return self.cmd.get_user_home(".config", "blacktool")
    ...
    def __rm_blackfury_binaries(self):
        for filename in ["blackfuryd", "ebrelayer", "blackgen"]:
    ...
    def __rm_run_env_files(self):
        self.__rm(self.cmd.get_user_home(".blackfuryd"))

        # Created by npx hardhat run scripts/devenv.ts and/or blacktool run-env
        self.__rm(self.project_dir("smart-contracts", "relayerdb"))  # peggy1 only
        self.__rm(self.project_dir("test", "integration", "offsideswaprelayerdb"))  # Probably obsolete on peggy2 TODO move to /tmp
    ...
    def clean(self):
        ...
        self.cmd.rmf(os.path.join(self.blacktool_dir, "build"))
        if on_peggy2_branch:
    ...
    def npm_install(self, path: str, disable_cache: bool = False):
        ...
        if self.cmd.exists(node_modules):
            cache_tag_file = os.path.join(node_modules, ".blacktool-cache-tag")
    ...
    def _ensure_build_dirs(self):
        for d in ["build", "build/repos", "build/generated"]:
            self.cmd.mkdir(os.path.join(self.blacktool_dir, d))
    ...
    def generate_python_protobuf_stubs(self):
        ...
        generated_dir = os.path.join(self.blacktool_dir, "build/generated")
        repos_dir = os.path.join(self.blacktool_dir, "build/repos")
        ...
        proto_files = [
            os.path.join(project_proto_dir, "blackfury/ethbridge/v1/tx.proto"),
            os.path.join(project_proto_dir, "blackfury/ethbridge/v1/query.proto"),
            os.path.join(project_proto_dir, "blackfury/ethbridge/v1/types.proto"),
            os.path.join(project_proto_dir, "blackfury/oracle/v1/network_descriptor.proto"),
            os.path.join(project_proto_dir, "blackfury/oracle/v1/types.proto"),
    ...
    def reset(self):
        ...
        self.__rm_hardhat_compiled_files()
        self.__rm_blackfury_binaries()
        self.__rm(os.path.join(self.cmd.get_user_home(), ".blackfuryd"))
        self.__rm_peggy2_compiled_go_stubs()
        self.__rm_run_env_files()
        self.npm_install(self.smart_contracts_dir)
        self.make_go_binaries_2()
    ...
    def pkill(self):
        for proc_name in ["node", "ebrelayer", "blackfuryd", "geth"]:
