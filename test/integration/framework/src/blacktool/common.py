...
# TODO Move to offsideswap.py
# TODO Refactoring in progress. This should be moved to offsideswap.py and only used for gas (float amount) + renamed
def black_format_amount(amount: Union[int, float], denom: str) -> str:
...
def blacktool_logger(name: Optional[str] = None):
    # Shortening "blacktool.eth" to "eth" results in a name clash with "eth" dependencies for which we want to disable logging...
    # if name is not None:
...
__log = blacktool_logger(__name__)
