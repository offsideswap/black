...
from blacktool import eth, truffle, hardhat, run_env, offsideswap, cosmos, command
from blacktool.offsideswap import FURY, CETH
from blacktool.common import *

# These are utilities to interact with running environment (running agains local ganache-cli/hardhat/blackfuryd).
...
blackfury_funds_for_transfer_peggy1 = 10**17  # fury
...
def get_env_ctx(cmd=None, env_file=None, env_vars=None):
    ...
    # Add any Offsideswap private keys to test keystore
    black_user_private_keys = ctx.cmd.project.read_peruser_config_file("black-keys")
    if black_user_private_keys:
        available_black_accounts = ctx.blackfury.keys_list()
        for name, address, mnemonic in [[e["name"], e["address"], e["mnemonic"].split(" ")] for e in black_user_private_keys]:
            existing_acct = [a for a in available_black_accounts if a["address"] == address]
            if not existing_acct:
                acct = ctx.blackfury.keys_add(name, mnemonic)
                assert acct["address"] == address, "Invalid address for black account {}".format(name)
    return ctx

def get_env_ctx_peggy2():
    cmd = run_env.Integrator()

    if "BLACKTOOL_ENV_FILE" in os.environ:
        # New-style format (hopefully unified)
        # This is for connecting to peggy2-tempnet etc.
        env_file = os.environ["BLACKTOOL_ENV_FILE"]
        env_vars = json.loads(cmd.read_text_file(env_file))
        global_mnemonic = env_vars.get("mnemonic", None)

        offsideswap_config = env_vars["offsideswap"]
        blackfury_url = offsideswap_config["rpc_url"]
        blackfury_chain_id = offsideswap_config["chain_id"]
        blackfuryd_home = offsideswap_config.get("home")
        ...
        fury_source = offsideswap_config.get("fury_source")
        if not fury_source:
            if "fury_source_mnemonic" in offsideswap_config:
                fury_source_mnemonic = offsideswap_config["fury_source_mnemonic"]
            ...
            if fury_source_mnemonic:
                ...
                fury_source = offsideswap.mnemonic_to_address(cmd, fury_source_mnemonic)
                blackfuryd = offsideswap.Blackfuryd(cmd, home=blackfuryd_home)
                if not [x for x in blackfuryd.keys_list() if x["address"] == fury_source]:
                    blackfuryd.keys_add(None, fury_source_mnemonic)
        ...
        # For either `blacktool run-env` or `devenv`
        ...
        blackfury_url = dot_env_vars["TCP_URL"]
        blackfury_chain_id = "localnet"  # TODO Mandatory, but not present either in environment_vars or dot_env_vars
        assert dot_env_vars["CHAINDIR"] == dot_env_vars["HOME"]
        blackfuryd_home = os.path.join(dot_env_vars["CHAINDIR"], ".blackfuryd")
    ...
    ceth_symbol = offsideswap.offsideswap_denom_hash(ethereum_network_descriptor, eth.NULL_ADDRESS)
    ...
    ctx = EnvCtx(cmd, w3_conn, ctx_eth, abi_provider, owner_address, blackfuryd_home, blackfury_url, blackfury_chain_id,
    ...
    # TODO These are set in run_env.py:Peggy2Environment.init_offsideswap(), specifically "blackfuryd tx ethbridge set-cross-chain-fee"
...
def get_env_ctx_peggy1(cmd=None, env_file=None, env_vars=None):
    ...
    if "CHAINNET" in env_vars:
        blackfury_chain_id = env_vars["CHAINNET"]
    elif deployment_name:
        blackfury_chain_id = deployment_name
    else:
        blackfury_chain_id = "localnet"
    ...
    generic_erc20_contract_name = "OffsideswapTestToken"
    if "SMART_CONTRACT_ARTIFACT_DIR" in env_vars:
        ...
        if deployment_name == "offsideswap-1":
            # Special case for Betanet because OffsideswapTestToken is not deployed there.
    ...
    blackfury_url = env_vars.get("BLACKFURY")  # Defaults to "tcp://localhost:26657"
    blackfuryd_home = None  # Implies default ~/.blackfuryd
    ...
    ctx = EnvCtx(cmd, w3_conn, ctx_eth, abi_provider, operator_address, blackfuryd_home, blackfury_url, blackfury_chain_id,
...
def black_addr_to_evm_arg(black_address):
    return black_address.encode("UTF-8")


class EnvCtx:
    def __init__(self, cmd: command.Command, w3_conn: web3.Web3, ctx_eth: eth.EthereumTxWrapper, abi_provider,
        operator: eth.Address, blackfuryd_home: str, blackfury_url: Optional[str], blackfury_chain_id: str,
        ...
        self.blackfury = offsideswap.Blackfuryd(self.cmd, home=blackfuryd_home, node=blackfury_url, chain_id=blackfury_chain_id)
        # Refactoring in progress: moving stuff into separate client that encapsulates things like url, home and chain_id
        self.blackfury_client = offsideswap.BlackfuryClient(self, self.blackfury, grpc_port=9090)
        ...
        self.offsideswap_ethbridge_admin_account = self.fury_source
    ...
    def tx_deploy_new_generic_erc20_token(self, deployer_addr: str, name: str, symbol: str, decimals: int, cosmosDenom: str = None) -> Contract:
        # return self.tx_deploy("OffsideswapTestToken", self.operator, [name, symbol, decimals])
        if on_peggy2_branch:
            # Use BridgeToken
            assert self.generic_erc20_contract == "BridgeToken"
            if cosmosDenom is None:
                cosmosDenom = "erc20denom"  # TODO Dummy variable since we're using BridgeToken instead of OffsideswapTestToken

            constructor_args = [name, symbol, decimals, cosmosDenom]
        else:
            # Use OffsideswapTestToken for TestNet and Devnet, and BridgeToken for Betanet
    ...
    def tx_bridge_bank_lock_eth(self, from_eth_acct, to_black_acct, amount):
        recipient = black_addr_to_evm_arg(to_black_acct)
        bridge_bank = self.get_bridge_bank_sc()
    ...
    def tx_bridge_bank_lock_erc20(self, token_addr, from_eth_acct, to_black_acct, amount):
        recipient = black_addr_to_evm_arg(to_black_acct)
        bridge_bank = self.get_bridge_bank_sc()
    ...
    def tx_bridge_bank_burn_erc20(self, token_addr: str, from_eth_acct: str, to_black_acct: str, amount: int) -> HexBytes:
        recipient = black_addr_to_evm_arg(to_black_acct)
        bridge_bank = self.get_bridge_bank_sc()
        # When transfering ERC20, the amount needs to be passed as argument, and the "message.value" should be 0
        tx_opts = {"value": 0}
    ...
    def tx_approve_and_lock(self, token_sc, from_eth_acct, to_black_acct, amount):
        bridge_bank_sc = self.get_bridge_bank_sc()
        txhash1 = self.tx_approve(token_sc, self.operator, bridge_bank_sc.address, amount)
        txhash2 = self.tx_bridge_bank_lock_erc20(token_sc.address, from_eth_acct, to_black_acct, amount)
        log.debug("tx_approve_and_lock: {} '{}' ({}) from {} to {}".format(amount, token_sc.functions.name().call(),
            token_sc.functions.symbol().call(), from_eth_acct, to_black_acct))
    ...
    def create_new_currency(self, symbol, name, decimals, amount, minted_tokens_recipient):
        ...
        1. Deploys a new instance of OffsideswapTestToken
        ...
        """
        assert self.generic_erc20_contract == "OffsideswapTestToken"  # Preserve compatibiliy with integration tests and inflate_tokens.sh
    ...
    def send_eth_from_ethereum_to_offsideswap(self, from_eth_addr, to_black_addr, amount):
        # recipient = to_black_addr.encode("UTF-8")
    ...
    # TODO Obsolete, use self.bridge_bank_lock_eth()
    def send_erc20_from_ethereum_to_offsideswap(self, from_eth_addr, dest_sichain_addr, erc20_token_addr, amount):
    ...
    # TODO Decouple; we want to use this with just "blackfuryd" running, move to Blackfuryd class?
    def create_offsideswap_addr(self, moniker: str = None,
        fund_amounts: Union[cosmos.Balance, cosmos.LegacyBalance, None] = None
    ) -> cosmos.Address:
        """
        Generates a new offsideswap address in test keyring. If moniker is given, uses it, otherwise
        generates a random one 'test-xxx'. If fund_amounts is given, the offsideswap funds are transferred
        from fury_source to the account before returning.
        """
        moniker = moniker or "test-" + random_string(20)
        acct = self.blackfury.keys_add(moniker)
        black_address = acct["address"]
        if fund_amounts:
            fund_amounts = cosmos.balance_normalize(fund_amounts)  # Convert from old format if neccessary
            fury_source_balances = self.get_offsideswap_balance(self.fury_source)
            for denom, required_amount in fund_amounts.items():
                ...
                assert available_amount >= required_amount, "Fury source {} would need {}, but only has {}".format(
                    self.fury_source, black_format_amount(required_amount, denom), black_format_amount(available_amount, denom))
            old_balances = self.get_offsideswap_balance(black_address)
            self.send_from_offsideswap_to_offsideswap(self.fury_source, black_address, fund_amounts)
            self.blackfury.wait_for_balance_change(black_address, old_balances, min_changes=fund_amounts)
            new_balances = self.get_offsideswap_balance(black_address)
            assert cosmos.balance_zero(cosmos.balance_sub(new_balances, fund_amounts))
        return black_address

    # TODO Clean up
    def send_from_offsideswap_to_offsideswap(self, from_black_addr: cosmos.Address, to_black_addr: cosmos.Address,
        amounts: cosmos.Balance
    ) -> Mapping:
        return self.blackfury.send(from_black_addr, to_black_addr, amounts)

    # TODO Clean up
    def get_offsideswap_balance(self, black_addr: cosmos.Address, height: Optional[int] = None,
        disable_log: bool = False, retries_on_error: Optional[int] = None, delay_on_error: int = 3
    ) -> cosmos.Balance:
        return self.blackfury.get_balance(black_addr, height=height, disable_log=disable_log,
            retries_on_error=retries_on_error, delay_on_error=delay_on_error)

    # TODO Clean up
    def get_current_block(self):
        return self.blackfury.get_current_block()

    # TODO Clean up
    def status(self):
        return self.blackfury.status()

    def eth_symbol_to_black_symbol(self, eth_token_symbol):
        assert not on_peggy2_branch
        # TODO offsideswap.use offsideswap_denom_hash() if on_peggy2_branch
        # E.g. "usdt" -> "cusdt"
    ...
    def bridge_bank_lock_eth(self, from_eth_acct, to_black_acct, amount):
        """ Sends ETH from Ethereum to Offsideswap (lock) """
        txhash = self.tx_bridge_bank_lock_eth(from_eth_acct, to_black_acct, amount)
        return self.eth.wait_for_transaction_receipt(txhash)

    def bridge_bank_lock_erc20(self, token_sc, from_eth_acct, to_black_acct, amount):
        txhash = self.tx_bridge_bank_lock_erc20(token_sc.address, from_eth_acct, to_black_acct, amount)
        return self.eth.wait_for_transaction_receipt(txhash)

    def bridge_bank_burn_erc20(self, token_sc: Contract, from_eth_acct: str, to_black_acct: str, amount: int) -> TxReceipt:
        txhash = self.tx_bridge_bank_burn_erc20(token_sc.address, from_eth_acct, to_black_acct, amount)
        return self.eth.wait_for_transaction_receipt(txhash)
    ...
    # For Offsideswap-native assets (fury) we need to use "burn".
    # Compare: smart-contracts/scripts/test/{sendLockTx.js OR sendBurnTx.js}
    # sendBurnTx is called when offsideswap_symbol == "fury", sendLockTx otherwise
    def send_from_ethereum_to_offsideswap(self, from_eth_acct: str, to_black_acct: str, amount: int, token_sc: Contract = None, isLock: bool = True) -> TxReceipt:
        if token_sc is None:
            # ETH transfer
            self.bridge_bank_lock_eth(from_eth_acct, to_black_acct, amount)
        else:
            # ERC20 token transfer
            self.approve_erc20_token(token_sc, from_eth_acct, amount)
            if isLock:
                self.bridge_bank_lock_erc20(token_sc, from_eth_acct, to_black_acct, amount)
            else:
                self.bridge_bank_burn_erc20(token_sc, from_eth_acct, to_black_acct, amount)
    ...
    def sanity_check(self):
        ...
        if on_peggy2_branch:
            ...
            assert (self.blackfury.chain_id != "offsideswap-testnet-1") or (bridge_bank_sc.address == "0x6CfD69783E3fFb44CBaaFF7F509a4fcF0d8e2835")
            assert (self.blackfury.chain_id != "offsideswap-devnet-1") or (bridge_bank_sc.address == "0x96DC6f02C66Bbf2dfbA934b8DafE7B2c08715A73")
            assert (self.blackfury.chain_id != "localnet") or (bridge_bank_sc.address == "0x30753E4A8aad7F8597332E813735Def5dD395028")
        ...
        available_accounts = self.blackfury.keys_list()
        ...
        if len(fury_source_account) != 1:
            raise Exception
        fury_source_balance = self.get_offsideswap_balance(self.fury_source).get(FURY, 0)
