...
mkdir -p $NETWORKDIR
blackgen network create localnet 1 $NETWORKDIR 192.168.1.2 $NETWORKDIR/network-definition.yml --keyring-backend test --mint-amount 999999000000000000000000000fury,1370000000000000000ibc/FEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACE
...
set_persistant_env_var BLACKFURYD_LOG $datadir/logs/blackfuryd.log $envexportfile

. $envexportfile

# now we have to add the validator key to the test keyring so the tests can send fury from validator1
echo "$MNEMONIC" | blackfuryd keys add $MONIKER --keyring-backend test --recover 
valoper=$(blackfuryd keys show -a --bech val $MONIKER --home $CHAINDIR/.blackfuryd --keyring-backend test)
blackfuryd add-genesis-validators $valoper --home $CHAINDIR/.blackfuryd

mkdir -p $datadir/logs
nohup $TEST_INTEGRATION_DIR/offsideswap_start_daemon.sh < /dev/null > $BLACKFURYD_LOG 2>&1 &
# we don't have a great way to make sure offsideswap itself has started
sleep 10
set_persistant_env_var BLACKFURYD_PID $! $envexportfile
bash $TEST_INTEGRATION_DIR/offsideswap_start_ebrelayer.sh
