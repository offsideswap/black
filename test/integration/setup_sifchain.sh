...
nohup $TEST_INTEGRATION_DIR/blackchain_start_daemon.sh < /dev/null > $BLACKFURYD_LOG 2>&1 &
# we don't have a great way to make sure blackchain itself has started
sleep 10
set_persistant_env_var BLACKFURYD_PID $! $envexportfile
bash $TEST_INTEGRATION_DIR/blackchain_start_ebrelayer.sh
