...
from blacktool import eth, test_utils
...
def bridge_bank_lock_eth(ctx, from_eth_acct, to_black_acct, amount):
    assert ctx.eth.get_eth_balance(from_eth_acct) > max_gas_required * max_gas_price, "Not enough gas for test"
    return ctx.bridge_bank_lock_eth(from_eth_acct, to_black_acct, amount)

def bridge_bank_lock_erc20(ctx, bridge_token, from_eth_acct, to_black_acct, amount):
    assert ctx.eth.get_eth_balance(from_eth_acct) > max_gas_required * max_gas_price, "Not enough gas for test"
    assert ctx.get_erc20_token_balance(bridge_token.address, from_eth_acct) >= amount, "Not enough tokens for test"
    return ctx.bridge_bank_lock_erc20(bridge_token, from_eth_acct, to_black_acct, amount)
...
def test_blocklist_eth(ctx):
    ...
    def assert_blocked(addr):
        ...
        try:
            bridge_bank_lock_eth(ctx, addr, to_black_acct, amount_to_send)
            assert False
    ...
    def assert_not_blocked(addr):
        ...
        assert balances_after.get(black_symbol, 0) == balances_before.get(black_symbol, 0) + amount_to_send
        ...
        assert entries[0].args["_to"] == test_utils.black_addr_to_evm_arg(to_black_acct)
...
def test_blocklist_erc20(ctx):
    ...
    black_token_symbol = ctx.eth_symbol_to_black_symbol(eth_token_symbol)
    ...
    def assert_blocked(addr):
        ...
        try:
            bridge_bank_lock_erc20(ctx, test_token, addr, to_black_acct, amount_to_send)
            assert False
        except Exception as e:
            assert is_blocklisted_exception(ctx, e)
    ...
    def assert_not_blocked(addr):
        ...
        assert balances_after.get(black_token_symbol, 0) == balances_before.get(black_token_symbol, 0) + amount_to_send
