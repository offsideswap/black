...
def test_create_new_claim(claimType):
    blackchain_address, blackchain_name = create_new_blackaddr_and_key()
    keyring_backend = 'test'
    chain_id = 'localnet'
    from_address = 'blackfuryadmin'
    amount = '100000000010000000fury'
    send_sample_fury(from_address, blackchain_address, amount, keyring_backend, chain_id, "")
    time.sleep(5)
    txnhash = (create_claim(blackchain_address, claimType, keyring_backend, chain_id))
    ...
    try:
        ...
        assert str(expectedOutputTagsList[1]) == blackchain_address
