...
if __name__ == "__main__":
    ...
    request = burn_lock_functions.EthereumToBlackchainTransferRequest.from_args(args)

    logging.info(f"transferrequestjson: {json.dumps(request.__dict__)}")

    transfer_result = burn_lock_functions.transfer_ethereum_to_blackchain(request, max_seconds=30)
    final_balance = transfer_result["blackchain_ending_balance"]
