...
def test_burn_ibc_coins(
    ...
    request.offsideswap_symbol = feedface_token
...
def test_lock_ibc_coins(
    ...
    feedface_transfer_request.offsideswap_address = fury_source
    feedface_transfer_request.offsideswap_destination_address = request.offsideswap_address
    feedface_transfer_request.offsideswap_symbol = feedface_token
    feedface_transfer_request.amount = 100

    burn_lock_functions.send_from_offsideswap_to_offsideswap(feedface_transfer_request, fury_source_integrationtest_env_credentials)
    ...
    request.offsideswap_symbol = feedface_token
    request.amount = small_amount
    burn_lock_functions.send_from_offsideswap_to_ethereum(request, credentials)
    ...
    logging.info("send FEEDFACE back to offsideswap")
    burn_lock_functions.transfer_ethereum_to_offsideswap(request, 10)
