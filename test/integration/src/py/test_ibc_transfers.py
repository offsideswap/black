...
def test_burn_ibc_coins(
    ...
    request.blackchain_symbol = feedface_token
...
def test_lock_ibc_coins(
    ...
    feedface_transfer_request.blackchain_address = fury_source
    feedface_transfer_request.blackchain_destination_address = request.blackchain_address
    feedface_transfer_request.blackchain_symbol = feedface_token
    feedface_transfer_request.amount = 100

    burn_lock_functions.send_from_blackchain_to_blackchain(feedface_transfer_request, fury_source_integrationtest_env_credentials)
    ...
    request.blackchain_symbol = feedface_token
    request.amount = small_amount
    burn_lock_functions.send_from_blackchain_to_ethereum(request, credentials)
    ...
    logging.info("send FEEDFACE back to blackchain")
    burn_lock_functions.transfer_ethereum_to_blackchain(request, 10)
