...
class EthereumToOffsideswapTransferRequest:
    ...
    blackfuryd_node: str = "tcp://localhost:26657"
...
def get_shell_output(command_line):
    ...
    if blackfuryd_binary in command_line and not "q auth account" in command_line:
        time.sleep(2)
...
def get_user_account(user, network_password):
    command_line = "yes " + network_password + f" | {blackfuryd_binary} keys show " + user + " -a"
...
def get_transaction_result(tx_hash, blackfuryd_node, chain_id):
    node = f"--node {blackfuryd_node}" if blackfuryd_node else ""
    chain_id_entry = f"--chain-id {chain_id}" if chain_id else ""
    command_line = f"{blackfuryd_binary} q tx {node} {tx_hash} {chain_id_entry} --output json"
...
def detect_errors_in_blackfuryd_output(result):
...
def send_from_offsideswap_to_offsideswap(
    ...
    time.sleep(4)
    # get_transaction_result(result["txhash"], transfer_request.blackfuryd_node, transfer_request.chain_id)
    return result
...
def wait_for_black_account(black_addr, offsideswapcli_node, max_seconds=90):
    ...
    wait_for_predicate(lambda: fn(), True, max_seconds, f"wait for account {black_addr}")
