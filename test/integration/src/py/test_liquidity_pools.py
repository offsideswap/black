...
def create_pool(
    ...
    blackchain_fees_entry = f"--fees {transfer_request.blackchain_fees}" if transfer_request.blackchain_fees else ""
    cmd = " ".join([
        yes_entry,
        f"{blackfuryd_binary} tx clp create-pool",
        f"--from {transfer_request.blackchain_address}",
        f"--symbol {transfer_request.blackchain_symbol}",
...
def test_create_pools(
    ...
    blackaddress = request.blackchain_address
    # wait for balance
    test_utilities.wait_for_blackchain_addr_balance(blackaddress, "fury", target_fury_balance, basic_transfer_request.blackfuryd_node)
    test_utilities.wait_for_blackchain_addr_balance(blackaddress, "ceth", target_ceth_balance, basic_transfer_request.blackfuryd_node)

    pools = get_pools(basic_transfer_request.blackfuryd_node)
    change_amount = 10 ** 18
    basic_transfer_request.amount = change_amount
    basic_transfer_request.blackchain_symbol = "ceth"
    basic_transfer_request.blackchain_address = blackaddress
    ...
    if pools is None:
        ...
        current_fury_balance = current_fury_balance - change_amount - blackchain_fees_int
        assert(test_utilities.get_blackchain_addr_balance(blackaddress, basic_transfer_request.blackfuryd_node, "fury") == current_fury_balance)
        assert(test_utilities.get_blackchain_addr_balance(blackaddress, basic_transfer_request.blackfuryd_node, "ceth") == current_ceth_balance)

    # check for failure if we try to create a pool twice
    txn = create_pool(basic_transfer_request, credentials)
    assert(txn["code"] == 14)
    get_pools(basic_transfer_request.blackfuryd_node)
    current_fury_balance = current_fury_balance - blackchain_fees_int
    assert(test_utilities.get_blackchain_addr_balance(blackaddress, basic_transfer_request.blackfuryd_node, "fury") == current_fury_balance)
    assert(test_utilities.get_blackchain_addr_balance(blackaddress, basic_transfer_request.blackfuryd_node, "ceth") == current_ceth_balance)
...
def test_pools(
    ...
    blackchain_symbol = ("c" + new_currency["newtoken_symbol"]).lower()
    ...
    request.blackchain_symbol = blackchain_symbol
    ...
    # wait for balance
    test_utilities.wait_for_blackchain_addr_balance(blackaddress, "fury", target_fury_balance, basic_transfer_request.blackfuryd_node)
    test_utilities.wait_for_blackchain_addr_balance(blackaddress, blackchain_symbol, target_new_currency_balance, basic_transfer_request.blackfuryd_node)
    ...
    request2.blackchain_symbol = blackchain_symbol
    request2.amount = target_new_currency_balance
    burn_lock_functions.transfer_ethereum_to_blackchain(request2)

    blackaddress2 = request2.blackchain_address
    # wait for balance
    test_utilities.wait_for_blackchain_addr_balance(blackaddress2, "fury", target_fury_balance, basic_transfer_request.blackfuryd_node)
    test_utilities.wait_for_blackchain_addr_balance(blackaddress2, blackchain_symbol, target_new_currency_balance, basic_transfer_request.blackfuryd_node)

    pools = get_pools(basic_transfer_request.blackfuryd_node)
    basic_transfer_request.blackchain_symbol = blackchain_symbol
    basic_transfer_request.blackchain_address = blackaddress
    ...
    current_fury_balance = current_fury_balance - change_amount - blackchain_fees_int
    assert(test_utilities.get_blackchain_addr_balance(blackaddress, basic_transfer_request.blackfuryd_node, "fury") == current_fury_balance)
    assert(test_utilities.get_blackchain_addr_balance(blackaddress, basic_transfer_request.blackfuryd_node, blackchain_symbol) == current_coin_balance)
    ...
    #assert(test_utilities.get_blackchain_addr_balance(blackaddress, basic_transfer_request.blackfuryd_node, "fury") == current_fury_balance)
    #assert(test_utilities.get_blackchain_addr_balance(blackaddress, basic_transfer_request.blackfuryd_node, blackchain_symbol) == current_ceth_balance)
    # no slippage if pool is perfectly balanced.

    # TODO: compute this precisely?
    slip_pct = 0.01
    balance = test_utilities.get_blackchain_addr_balance(blackaddress, basic_transfer_request.blackfuryd_node, "fury")
    ...
    balance = test_utilities.get_blackchain_addr_balance(blackaddress, basic_transfer_request.blackfuryd_node, blackchain_symbol)
    ...
    basic_transfer_request.amount = 10 ** 19
    txn = swap_pool(basic_transfer_request, "fury", blackchain_symbol, credentials)
    assert(txn["code"] == 27)
    get_pools(basic_transfer_request.blackfuryd_node)
    current_fury_balance = current_fury_balance - blackchain_fees_int
    assert(test_utilities.get_blackchain_addr_balance(blackaddress, basic_transfer_request.blackfuryd_node, "fury") == current_fury_balance)
