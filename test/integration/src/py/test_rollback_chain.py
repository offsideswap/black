...
from test_utilities import get_shell_output, get_blackchain_addr_balance, \
    advance_n_ethereum_blocks, n_wait_blocks, \
    send_from_ethereum_to_blackchain
from test_utilities import wait_for_blackchain_addr_balance, \
    get_required_env_var, \
    EthereumToBlackchainTransferRequest
...
def test_rollback_chain(source_ethereum_address, solidity_json_path):
    ...
    request = EthereumToBlackchainTransferRequest(
        blackchain_address=new_account["address"],
    ...
    burn_lock_functions.transfer_ethereum_to_blackchain(request, 50)
    ...
    initial_user_balance = get_blackchain_addr_balance(new_addr, "", request.blackchain_symbol)
    logging.info(f"initial_user_balance {initial_user_balance}")

    transfer_1 = send_from_ethereum_to_blackchain(transfer_request=request)
    ...
    # the transaction should not have happened on the blackchain side yet
    ...
    second_user_balance = get_blackchain_addr_balance(new_addr, "", request.blackchain_symbol)
    ...
    balance_after_sleep = get_blackchain_addr_balance(new_addr, "", request.blackchain_symbol)
    logging.info(f"get_blackchain_addr_balance after sleep is {balance_after_sleep} for {new_addr}")

    expected_balance = initial_user_balance + request.amount
    logging.info(f"look for a balance of {expected_balance}")
    wait_for_blackchain_addr_balance(new_addr, request.blackchain_symbol, expected_balance, "")
