...
@pytest.fixture
def smart_contracts_dir(blackfury_base_dir):
    return test_utilities.get_optional_env_var("SMART_CONTRACTS_DIR", os.path.join(blackfury_base_dir, "smart-contracts"))


@pytest.fixture
def blackfury_base_dir():
...
@pytest.fixture
def blackfuryd_node():
    return test_utilities.get_optional_env_var("BLACKFURY", None)
...
@pytest.fixture
def is_ropsten_testnet(blackfuryd_node):
    """if blackfury_clinode is set, we're talking to ropsten/sandpit"""
    return blackfuryd_node
...
@pytest.fixture
def ganache_keys_file(blackfury_base_dir):
    return test_utilities.get_optional_env_var(
        "GANACHE_KEYS_FILE",
        os.path.join(blackfury_base_dir, "test/integration/vagrant/data/ganachekeys.json")
    )
...
def ethbridge_module_address():
    """The hardcoded address of the blackfury ethbridge module"""
    return "black1l3dftf499u4gvdeuuzdl2pgv4f0xdtnuuwlzp8"
...
def ctx(request):
    ...
    with blacktool.test_utils.get_test_env_ctx() as ctx:
        yield ctx
