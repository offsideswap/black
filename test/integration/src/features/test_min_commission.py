...
# Run test/integration/framework/blacktool venv
# Prepare blackfuryd binaries
# - Compile blackfuryd binary for v0.14.0 and put them into test/integration/framework/build/versions/0.14.0/blackfuryd
# - Compile blackfuryd binary for v0.15.0-rc.1 and put them into test/integration/framework/build/versions/0.15.0-rc.1/blackfuryd
...
# Kill off any blackfuryd processes running from before
def pkill(cmd):
...
def get_binary_for_version(label):
    return project_dir("test", "integration", "framework", "build", "versions", label, "blackfuryd")
...
def create_environment(cmd, version, commission_rate=0.06, commission_max_rate=0.10, commission_max_change_rate=0.05,
    ...
    home_root = "/tmp/blacktool.tmp/test_min_commission"
    cmd.rmdir(home_root)
    cmd.mkdir(home_root)
    ...
    env = environments.BlackfurydEnvironment(cmd, blackfuryd_home_root=home_root)
...
def delegate(env, from_index, to_index, amount):
    ...
    blackfuryd_to = env._blackfuryd_for(to_validator_node_info)
    blackfuryd_from_to = env._blackfuryd_for(from_validator_node_info, to_node_info=to_validator_node_info)
    validator_addr = blackfuryd_to.get_val_address(to_validator_node_info["admin_addr"])
...
def test_min_commission_modify_existing_validator_24h(cmd: command.Command):
    ...
    blackfuryd0 = env._blackfuryd_for(env.node_info[0])
    admin0_addr = env.node_info[0]["admin_addr"]
    blackfuryd1 = env._blackfuryd_for(env.node_info[1])
    admin1_addr = env.node_info[1]["admin_addr"]
    blackfuryd2 = env._blackfuryd_for(env.node_info[2])
    admin2_addr = env.node_info[2]["admin_addr"]

    validators = blackfuryd0.query_staking_validators()
...
def test_min_commission_upgrade_handler(cmd: command.Command):
    def test_case(pre_upgrade_commission_rate, pre_upgrade_commission_max_rate, expected_commission_rate,
        ...
        if should_succeed:
            ...
            # TODO In case of invalid validator setup (commission_rate > commission_max_rate) blackfuryd does not start
        ...
        upgrade_height = blackfuryd.get_current_block() + 15  # 15 * 5 = 75s > 60s

        commission_rates_before = exactly_one(blackfuryd.query_staking_validators())["commission"]["commission_rates"]
        ...
        commission_rates_after = exactly_one(blackfuryd.query_staking_validators())["commission"]["commission_rates"]
...
def test_max_voting_power(cmd: command.Command):
    def test_case(from_validator_index, to_validator_index, amount, should_succeed):
        ...
        validator_powers_before = [int(x["tokens"]) for x in blackfuryd.query_staking_validators()]
        ...
        validator_powers_after = [int(x["tokens"]) for x in blackfuryd.query_staking_validators()]

        if should_succeed:
            assert_no_exception(exception)
            # Check actual vs. expected validator powers.
            # Note: this assertion might fail if "blackfuryd query staking validators" returns a list in different order.
    ...
    test_case(akasha, black, 100, False)  # Current (and projected) voting power too big
    test_case(black, akasha, 100, True)
    test_case(black, akasha, 10**23, False)  # Projected voting power too big
