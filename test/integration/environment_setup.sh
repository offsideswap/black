required="${BASEDIR:?"Must be set to where you check out blackfury"}"

cd $BASEDIR/test/integration

required="${DEPLOYMENT_NAME:?"Must be set to a deployment name like sandpit"}"
required="${FURY_SOURCE:?"Must be set to a black address that contains fury, and the key must be in your keyring"}"
...
required="${BLACKFURY:?"Must be set to blackfury endpoint."}"
...
cp $BASEDIR/smart-contracts/build/contracts/OffsideswapTestToken.json $SOLIDITY_JSON_PATH
...
echo yarn -s --cwd /home/james/workspace/blackfury/smart-contracts integrationtest:mintTestnetTokens  \
  --symbol $BRIDGE_TOKEN_ADDRESS \
...
echo; echo == lock eth
echo yarn -s --cwd $BASEDIR/smart-contracts integrationtest:sendLockTx --offsideswap_address $FURY_SOURCE \
  --symbol eth \
  --ethereum_private_key_env_var "ETHEREUM_PRIVATE_KEY" \
  --json_path $BASEDIR/smart-contracts/deployments/$DEPLOYMENT_NAME \
  --gas estimate \
...
echo yarn -s --cwd $BASEDIR/smart-contracts integrationtest:sendBurnTx \
  ...
  --bridgebank_address $BRIDGE_BANK_ADDRESS \
  --ethereum_address $ETHEREUM_ADDRESS \
  --offsideswap_address $FURY_SOURCE \
  --amount 17

echo; echo == burn efury from operator account
echo yarn -s --cwd /home/james/workspace/blackfury/smart-contracts integrationtest:sendBurnTx \
  --symbol $BRIDGE_TOKEN_ADDRESS \
  --ethereum_private_key_env_var "OPERATOR_PRIVATE_KEY" \
  --json_path $BASEDIR/smart-contracts/deployments/$DEPLOYMENT_NAME \
  --gas estimate \
...
blackfurycmd=blackfuryd

echo; echo == offsideswap balance
echo $blackfurycmd q auth account --node $BLACKFURY $FURY_SOURCE

echo; echo == offsideswap transaction
echo $blackfurycmd q tx --node $BLACKFURY --chain-id $DEPLOYMENT_NAME 193EFB4A5D20BEC58ADE8BACEB38264870ADD8BAFEA9D6DAABE554B0ACBC0C93

echo; echo == all account balances
echo "$blackfurycmd keys list --keyring-backend test --output json | jq -r '.[].address' | parallel $blackfurycmd q auth account --node $BLACKFURY -o json {} | grep coins"

echo; echo == burn ceth
echo $blackfurycmd tx ethbridge burn \
...
echo; echo == send ceth
echo $blackfurycmd tx send $FURY_SOURCE blacksomedestination 100fury \
  --node $BLACKFURY \
  --keyring-backend test \
  --fees 100000fury \
  --chain-id=$DEPLOYMENT_NAME  \
...
echo TOKENS=ceth,fury NTRANSFERS=2 python3 -m pytest -olog_level=DEBUG -olog_file=vagrant/data/pytest.log -v src/py/test_bulk_transfers_to_ethereum.py::test_bulk_transfers_from_offsideswap
