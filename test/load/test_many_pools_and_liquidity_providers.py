...
# Example usage:
# cd test/load
# ../integration/framework/blacktool venv
...
#
# (4) Use parameter rpc.laddr for blackfuryd start instead of self.node
...
class Test:
    def __init__(self, cmd: command.Command, prj: project.Project, blackfuryd_home_root: str):
        self.cmd = cmd
        self.prj = prj
        ...
        self.blackfuryd_home_root = blackfuryd_home_root

        self.env = None
        self.custom_wallet_mnemonics = [
            # black1rruvw03utshn7ry3emeqf2gzkg6eap6hu5shun
        ...
        self.blackfuryd = None
        self.blackfuryd_client = None
    ...
    def setup(self):
        ...
        if self.wallets_dir is not None:
            ...
            client_home = os.path.join(self.blackfuryd_home_root, "blackfuryd-client")
        ...
        for i in range(self.number_of_wallets):
            ...
            if predefined_wallets:
                ...
                addr = blackfuryd_client.create_addr(mnemonic=mnemonic)
        ...
        for addr, denoms in wallets.items():
            account_number, account_sequence = blackfuryd.get_acct_seq(addr)
            for denom in denoms:
        ...
        self.blackfuryd_client = blackfuryd_client

    def run(self):
        blackfuryd = self.blackfuryd
        blackfuryd_client = self.blackfuryd_client
        ...
        current_block = blackfuryd_client.get_current_block()
        ...
        # TODO blackfuryd query reward params --node --chain-id (check if/when implemented)
    ...
    # TODO Refactor - move to Blackfuryd
    def wait_for_block(self, block_number: int) -> float:
        blackfuryd = self.blackfuryd
        current_block = blackfuryd.get_current_block()
        prev_block = None
            ...
            if self.block_results_offset is not None:
                if (prev_block is None) or (current_block != prev_block):
                    ...
                    try:
                        ...
                        blk = blackfuryd.get_block_results(height=height)
                        histogram = {}
    ...
    def check_actual_liquidity_providers(self, blackfuryd, clp_admin, wallets):
        actual_lp_providers = {}
        for denom in self.tokens:
            for lp in blackfuryd.query_clp_liquidity_providers(denom):
...
def run_test_case(args):
    ...
    blackfuryd_home_root = cmd.tmpdir("blacktool-test.tmp")
    cmd.rmdir(blackfuryd_home_root)
    test = Test(cmd, prj, blackfuryd_home_root=blackfuryd_home_root)
    ...
    try:
        ...
        try:
            ...
            balance = cosmos.balance_format(test.blackfuryd[0].get_balance(addr))
