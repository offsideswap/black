...
func TestNewLegacyQuerier(t *testing.T) {
	...
	keeper := blackapp.DispensationKeeper
	...
	qp, errRes := blackapp.LegacyAmino().MarshalJSON(&queryRecName)
	...
	err = blackapp.LegacyAmino().UnmarshalJSON(res, &dr)
	assert.Error(t, err)
...
func TestQueryRecordsName(t *testing.T) {
	...
	GenerateQueryData(blackapp, ctx, name, outList)
	keeper := blackapp.DispensationKeeper
	querier := dispensationkeeper.NewLegacyQuerier(keeper)
	queryRecName := types.QueryRecordsByDistributionNameRequest{
		DistributionName: name,
...
func TestQueryRecordsAddr(t *testing.T) {
	...
	qp, errRes := blackapp.LegacyAmino().MarshalJSON(&quereyRecName)
	require.NoError(t, errRes)
	query.Path = ""
	query.Data = qp
