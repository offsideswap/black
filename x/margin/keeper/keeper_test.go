...
func TestKeeper_Borrow(t *testing.T) {
	...
	borrowTests := []struct {
		...
		},
		{
			...
			decimals:         18,
			to:               "fury",
			address:          "black1azpar20ck9lpys89r8x7zc8yu0qzgvtp48ng5v",
			collateralAmount: sdk.NewUint(10000),
			custodyAmount:    sdk.NewUint(1000),
			leverage:         sdk.NewDec(1),
			health:           sdk.NewDec(1),
	...
	for _, tt := range borrowTests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.fundedAccount {
				...
				err := blackapp.AddCoinsToAccount(types.ModuleName, app.BankKeeper, ctx, signer, sdk.NewCoins(nativeCoin))
				require.Nil(t, err)
			} else {
				address = tt.address
			}
...
func TestWhitelist(t *testing.T) {
	...
	marginKeeper.WhitelistAddress(ctx, "black123")
	is := marginKeeper.IsWhitelisted(ctx, "black123")
	require.True(t, is)
	whitelist, _, err := marginKeeper.GetWhitelist(ctx, nil)
	require.NoError(t, err)
	require.Equal(t, []string{"black123"}, whitelist)
}
