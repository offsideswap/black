	...
	for _, tt := range table {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.fundedAccount {
				...
				err = blackapp.AddCoinsToAccount(types.ModuleName, app.BankKeeper, ctx, _signer, sdk.NewCoins(nativeCoin, externalCoin))
				require.Nil(t, err)
			} else {
	...
	for _, tt := range table {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.fundedAccount {
				...
				_signer := clptest.GenerateAddress(clptest.AddressKey1)
				address = _signer.String()
				err = blackapp.AddCoinsToAccount(types.ModuleName, app.BankKeeper, ctx, _signer, sdk.NewCoins(nativeCoin))
				require.NoError(t, err)
...
func TestKeeper_ForceClose(t *testing.T) {
	table := []struct {
	...
	}{
		{
			...
			msgForceClose: types.MsgForceClose{
				Signer:     "black1azpar20ck9lpys89r8x7zc8yu0qzgvtp48ng5v",
				MtpAddress: "black1azpar20ck9lpys89r8x7zc8yu0qzgvtp48ng5v",
			},
			msgOpen: types.MsgOpen{
				CollateralAsset: "xxx",
				BorrowAsset:     "xxx",
			...
			ctx, app := test.CreateTestAppMarginFromGenesis(false, func(app *blackapp.OffsideswapApp, genesisState blackapp.GenesisState) blackapp.GenesisState {
				...
				gs3 := &types.GenesisState{
					Params: &types.Params{
						...
						ForceCloseFundAddress:                    "black1syavy2npfyt9tcncdtsdzf7kny9lh777yqc2nd",
						IncrementalInterestPaymentFundPercentage: sdk.NewDecWithPrec(1, 1),
						IncrementalInterestPaymentFundAddress:    "black1syavy2npfyt9tcncdtsdzf7kny9lh777yqc2nd",
...
func TestKeeper_OpenClose(t *testing.T) {
	...
	for _, tt := range table {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			err = blackapp.AddCoinsToAccount(types.ModuleName, app.BankKeeper, ctx, signer, sdk.NewCoins(nativeCoin, externalCoin))
			require.Nil(t, err)
...
func TestKeeper_OpenThenClose(t *testing.T) {
	...
	ctx, app := test.CreateTestAppMarginFromGenesis(false, func(app *blackapp.OffsideswapApp, genesisState blackapp.GenesisState) blackapp.GenesisState {
		...
		balances := []banktypes.Balance{
			{
				Address: "black1pjm228rsgwqf23arkx7lm9ypkyma7mzr3y2n85",
				Coins: sdk.Coins{
